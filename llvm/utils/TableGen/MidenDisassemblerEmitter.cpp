//===- MidenDisassemblerEmitter.cpp - Disassembler tables -*- C++ -*-===//
//
...
void emitMidenDisassemblerTables(
  ...
  for (unsigned I = 0; I != NumberedInstructions.size(); ++I) {
    ...
    // All miden instructions have a StackBased field of type string, we only
  ...
  OS << "#include \"MCTargetDesc/MidenMCTargetDesc.h\"\n";
  OS << "\n";
  OS << "namespace llvm {\n\n";
  OS << "static constexpr int MidenInstructionTableSize = ";
  OS << MidenInstructionTableSize << ";\n\n";
  OS << "enum EntryType : uint8_t { ";
  OS << "ET_Unused, ET_Prefix, ET_Instruction };\n\n";
  OS << "struct MidenInstruction {\n";
  OS << "  uint16_t Opcode;\n";
  OS << "  EntryType ET;\n";
  ...
  for (auto &PrefixPair : OpcodeTable) {
    if (PrefixPair.second.empty())
      continue;
    OS << "MidenInstruction InstructionTable" << PrefixPair.first;
    OS << "[] = {\n";
    for (unsigned I = 0; I < MidenInstructionTableSize; I++) {
  ...
  OS << "struct { uint8_t Prefix; const MidenInstruction *Table; }\n";
  OS << "PrefixTable[] = {\n";
  for (auto &PrefixPair : OpcodeTable) {
