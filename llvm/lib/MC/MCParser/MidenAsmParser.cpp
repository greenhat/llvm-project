//===- MidenAsmParser.cpp - Miden Assembly Parser -----------------------------===//
//
...
// --
//
// Note, this is for Miden, the binary format (analogous to ELF), not Miden,
...
// MCTargetStreamer, the ones that depend on MidenTargetStreamer are in
// MidenAsmParser.
//
//===----------------------------------------------------------------------===//
...
class MidenAsmParser : public MCAsmParserExtension {
  MCAsmParser *Parser = nullptr;
  MCAsmLexer *Lexer = nullptr;

  template<bool (MidenAsmParser::*HandlerMethod)(StringRef, SMLoc)>
  void addDirectiveHandler(StringRef Directive) {
    MCAsmParser::ExtensionDirectiveHandler Handler = std::make_pair(
        this, HandleDirective<MidenAsmParser, HandlerMethod>);

    getParser().addDirectiveHandler(Directive, Handler);
  }

public:
  MidenAsmParser() { BracketExpressionsSupported = true; }
  ...
  void Initialize(MCAsmParser &P) override {
    ...
    addDirectiveHandler<&MidenAsmParser::parseSectionDirectiveText>(".text");
    addDirectiveHandler<&MidenAsmParser::parseSectionDirectiveData>(".data");
    addDirectiveHandler<&MidenAsmParser::parseSectionDirective>(".section");
    addDirectiveHandler<&MidenAsmParser::parseDirectiveSize>(".size");
    addDirectiveHandler<&MidenAsmParser::parseDirectiveType>(".type");
    addDirectiveHandler<&MidenAsmParser::ParseDirectiveIdent>(".ident");
    addDirectiveHandler<
      &MidenAsmParser::ParseDirectiveSymbolAttribute>(".weak");
    addDirectiveHandler<
      &MidenAsmParser::ParseDirectiveSymbolAttribute>(".local");
    addDirectiveHandler<
      &MidenAsmParser::ParseDirectiveSymbolAttribute>(".internal");
    addDirectiveHandler<
      &MidenAsmParser::ParseDirectiveSymbolAttribute>(".hidden");
  }
  ...
  uint32_t parseSectionFlags(StringRef FlagStr, bool &Passive, bool &Group) {
    ...
    for (char C : FlagStr) {
      ...
      case 'G':
        ...
        break;
      case 'T':
        flags |= miden::MIDEN_SEG_FLAG_TLS;
        break;
      case 'S':
        flags |= miden::MIDEN_SEG_FLAG_STRINGS;
        break;
      default:
        return -1U;
      }
  ...
  bool parseSectionDirective(StringRef, SMLoc loc) {
    ...
    auto Kind = StringSwitch<Optional<SectionKind>>(Name)
                    ...
                    // See use of .init_array in MidenObjectWriter and
                    // TargetLoweringObjectFileMiden
    ...
    // TODO: Parse UniqueID
    MCSectionMiden *WS = getContext().getMidenSection(
  ...
  bool parseDirectiveType(StringRef, SMLoc) {
    ...
    if (!Lexer->is(AsmToken::Identifier))
      return error("Expected label after .type directive, got: ",
                   Lexer->getTok());
    auto MidenSym = cast<MCSymbolMiden>(
    ...
    if (TypeName == "function") {
      ...
      auto *Current =
          cast<MCSectionMiden>(getStreamer().getCurrentSection().first);
      if (Current->getGroup())
      ...
      return error("Unknown Miden symbol type: ", Lexer->getTok());
    Lex();
...
MCAsmParserExtension *createMidenAsmParser() {
  return new MidenAsmParser;
}
