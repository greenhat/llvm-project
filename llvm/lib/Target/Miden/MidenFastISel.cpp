//===-- MidenFastISel.cpp - Miden FastISel implementation -----===//
//
...
///
/// \file
/// This file defines the Miden-specific support for the FastISel
/// class. Some of the target-specific code is generated by tablegen in the file
/// MidenGenFastISel.inc, which is #included here.
///
/// TODO: kill flags
///
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "miden-fastisel"

namespace {

class MidenFastISel final : public FastISel {
...
public:
  // Backend specific FastISel code.
  MidenFastISel(FunctionLoweringInfo &FuncInfo,
                const TargetLibraryInfo *LibInfo)
      : FastISel(FuncInfo, LibInfo, /*SkipTargetIndependentISel=*/true) {
    Subtarget = &FuncInfo.MF->getSubtarget<MidenSubtarget>();
    Context = &FuncInfo.Fn->getContext();
  }

  bool fastSelectInstruction(const Instruction *I) override;

#include "MidenGenFastISel.inc"
};

} // end anonymous namespace

bool MidenFastISel::computeAddress(const Value *Obj, Address &Addr) {
  ...
  case Instruction::GetElementPtr: {
    ...
    // Non-inbounds geps can wrap; miden's offsets can't.
...
void MidenFastISel::materializeLoadStoreOperands(Address &Addr) {
  if (Addr.isRegBase()) {
    unsigned Reg = Addr.getReg();
    if (Reg == 0) {
      Reg = createResultReg(Subtarget->hasAddr64() ? &Miden::I64RegClass
                                                   : &Miden::I32RegClass);
      unsigned Opc =
          Subtarget->hasAddr64() ? Miden::CONST_I64 : Miden::CONST_I32;
...
void MidenFastISel::addLoadStoreOperands(const Address &Addr,
...
unsigned MidenFastISel::maskI1Value(unsigned Reg, const Value *V) {
  return zeroExtendToI32(Reg, V, MVT::i1);
}

unsigned MidenFastISel::getRegForI1Value(const Value *V, const BasicBlock *BB,
                                         bool &Not) {
...
unsigned MidenFastISel::zeroExtendToI32(unsigned Reg, const Value *V,
                                        MVT::SimpleValueType From) {
  if (Reg == 0)
    return 0;
  ...
  Register Imm = createResultReg(&Miden::I32RegClass);
  BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(Miden::CONST_I32),
          Imm)
      .addImm(~(~uint64_t(0) << MVT(From).getSizeInBits()));

  Register Result = createResultReg(&Miden::I32RegClass);
  BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(Miden::AND_I32),
          Result)
      .addReg(Reg)
      .addReg(Imm);
...
unsigned MidenFastISel::signExtendToI32(unsigned Reg, const Value *V,
                                        MVT::SimpleValueType From) {
  if (Reg == 0)
    return 0;
  ...
  Register Left = createResultReg(&Miden::I32RegClass);
  BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(Miden::SHL_I32),
          Left)
      .addReg(Reg)
      .addReg(Imm);

  Register Right = createResultReg(&Miden::I32RegClass);
  BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(Miden::SHR_S_I32),
          Right)
      .addReg(Left)
      .addReg(Imm);
...
unsigned MidenFastISel::zeroExtend(unsigned Reg, const Value *V,
                                   MVT::SimpleValueType From,
                                   MVT::SimpleValueType To) {
  if (To == MVT::i64) {
    if (From == MVT::i64)
    ...
    Register Result = createResultReg(&Miden::I64RegClass);
    BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc,
            TII.get(Miden::I64_EXTEND_U_I32), Result)
        .addReg(Reg);
    return Result;
  }
...
unsigned MidenFastISel::signExtend(unsigned Reg, const Value *V,
                                   MVT::SimpleValueType From,
                                   MVT::SimpleValueType To) {
  if (To == MVT::i64) {
    if (From == MVT::i64)
    ...
    Register Result = createResultReg(&Miden::I64RegClass);
    BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc,
            TII.get(Miden::I64_EXTEND_S_I32), Result)
        .addReg(Reg);
    return Result;
  }
...
unsigned MidenFastISel::getRegForUnsignedValue(const Value *V) {
...
unsigned MidenFastISel::getRegForSignedValue(const Value *V) {
  MVT::SimpleValueType From = getSimpleType(V->getType());
  MVT::SimpleValueType To = getLegalType(From);
  Register VReg = getRegForValue(V);
  if (VReg == 0)
...
unsigned MidenFastISel::getRegForPromotedValue(const Value *V, bool IsSigned) {
  return IsSigned ? getRegForSignedValue(V) : getRegForUnsignedValue(V);
}

unsigned MidenFastISel::notValue(unsigned Reg) {
  assert(MRI.getRegClass(Reg) == &Miden::I32RegClass);

  Register NotReg = createResultReg(&Miden::I32RegClass);
  BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(Miden::EQZ_I32),
          NotReg)
      .addReg(Reg);
  return NotReg;
}

unsigned MidenFastISel::copyValue(unsigned Reg) {
  Register ResultReg = createResultReg(MRI.getRegClass(Reg));
  BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(Miden::COPY),
          ResultReg)
      .addReg(Reg);
  return ResultReg;
}

unsigned MidenFastISel::fastMaterializeAlloca(const AllocaInst *AI) {
  DenseMap<const AllocaInst *, int>::iterator SI =
      FuncInfo.StaticAllocaMap.find(AI);

  if (SI != FuncInfo.StaticAllocaMap.end()) {
    Register ResultReg = createResultReg(
        Subtarget->hasAddr64() ? &Miden::I64RegClass : &Miden::I32RegClass);
    unsigned Opc = Subtarget->hasAddr64() ? Miden::COPY_I64 : Miden::COPY_I32;
...
unsigned MidenFastISel::fastMaterializeConstant(const Constant *C) {
  if (const GlobalValue *GV = dyn_cast<GlobalValue>(C)) {
    ...
    Register ResultReg = createResultReg(
        Subtarget->hasAddr64() ? &Miden::I64RegClass : &Miden::I32RegClass);
    unsigned Opc = Subtarget->hasAddr64() ? Miden::CONST_I64 : Miden::CONST_I32;
    BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(Opc), ResultReg)
        .addGlobalAddress(GV);
    return ResultReg;
  }
...
bool MidenFastISel::fastLowerArguments() {
  ...
  for (auto const &Arg : F->args()) {
    ...
    case MVT::i16:
    case MVT::i32:
      Opc = Miden::ARGUMENT_i32;
      RC = &Miden::I32RegClass;
      break;
    case MVT::i64:
      Opc = Miden::ARGUMENT_i64;
      RC = &Miden::I64RegClass;
      break;
    case MVT::f32:
      Opc = Miden::ARGUMENT_f32;
      RC = &Miden::F32RegClass;
      break;
    case MVT::f64:
      Opc = Miden::ARGUMENT_f64;
      RC = &Miden::F64RegClass;
      break;
    case MVT::v16i8:
      Opc = Miden::ARGUMENT_v16i8;
      RC = &Miden::V128RegClass;
      break;
    case MVT::v8i16:
      Opc = Miden::ARGUMENT_v8i16;
      RC = &Miden::V128RegClass;
      break;
    case MVT::v4i32:
      Opc = Miden::ARGUMENT_v4i32;
      RC = &Miden::V128RegClass;
      break;
    case MVT::v2i64:
      Opc = Miden::ARGUMENT_v2i64;
      RC = &Miden::V128RegClass;
      break;
    case MVT::v4f32:
      Opc = Miden::ARGUMENT_v4f32;
      RC = &Miden::V128RegClass;
      break;
    case MVT::v2f64:
      Opc = Miden::ARGUMENT_v2f64;
      RC = &Miden::V128RegClass;
      break;
    case MVT::funcref:
      Opc = Miden::ARGUMENT_funcref;
      RC = &Miden::FUNCREFRegClass;
      break;
    case MVT::externref:
      Opc = Miden::ARGUMENT_externref;
      RC = &Miden::EXTERNREFRegClass;
      break;
    default:
      return false;
    }
...
bool MidenFastISel::selectCall(const Instruction *I) {
  ...
  unsigned Opc = IsDirect ? Miden::CALL : Miden::CALL_INDIRECT;
  ...
  if (!IsVoid) {
    ...
    case MVT::i16:
    case MVT::i32:
      ResultReg = createResultReg(&Miden::I32RegClass);
      break;
    case MVT::i64:
      ResultReg = createResultReg(&Miden::I64RegClass);
      break;
    case MVT::f32:
      ResultReg = createResultReg(&Miden::F32RegClass);
      break;
    case MVT::f64:
      ResultReg = createResultReg(&Miden::F64RegClass);
      break;
    case MVT::v16i8:
      ResultReg = createResultReg(&Miden::V128RegClass);
      break;
    case MVT::v8i16:
      ResultReg = createResultReg(&Miden::V128RegClass);
      break;
    ...
    case MVT::v2f64:
      ...
      break;
    case MVT::funcref:
      ResultReg = createResultReg(&Miden::FUNCREFRegClass);
      break;
    case MVT::externref:
      ResultReg = createResultReg(&Miden::EXTERNREFRegClass);
      break;
    default:
      return false;
    }
  ...
  if (IsDirect) {
    ...
    MCSymbolMiden *Table = Miden::getOrCreateFunctionTableSymbol(
    ...
    // See also: MidenISelLowering.cpp: LowerCallResults
    if (Subtarget->hasAddr64()) {
      auto Wrap = BuildMI(*FuncInfo.MBB, std::prev(FuncInfo.InsertPt), DbgLoc,
                          TII.get(Miden::I32_WRAP_I64));
      Register Reg32 = createResultReg(&Miden::I32RegClass);
...
bool MidenFastISel::selectSelect(const Instruction *I) {
  ...
  case MVT::i16:
  case MVT::i32:
    Opc = Miden::SELECT_I32;
    RC = &Miden::I32RegClass;
    break;
  case MVT::i64:
    Opc = Miden::SELECT_I64;
    RC = &Miden::I64RegClass;
    break;
  case MVT::f32:
    Opc = Miden::SELECT_F32;
    RC = &Miden::F32RegClass;
    break;
  case MVT::f64:
    Opc = Miden::SELECT_F64;
    RC = &Miden::F64RegClass;
    break;
  case MVT::funcref:
    Opc = Miden::SELECT_FUNCREF;
    RC = &Miden::FUNCREFRegClass;
    break;
  case MVT::externref:
    Opc = Miden::SELECT_EXTERNREF;
    RC = &Miden::EXTERNREFRegClass;
    break;
  default:
    return false;
...
bool MidenFastISel::selectTrunc(const Instruction *I) {
  ...
  if (Trunc->getOperand(0)->getType()->isIntegerTy(64)) {
    Register Result = createResultReg(&Miden::I32RegClass);
    BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc,
            TII.get(Miden::I32_WRAP_I64), Result)
        .addReg(Reg);
    Reg = Result;
  }
...
bool MidenFastISel::selectZExt(const Instruction *I) {
...
bool MidenFastISel::selectSExt(const Instruction *I) {
...
bool MidenFastISel::selectICmp(const Instruction *I) {
  ...
  case ICmpInst::ICMP_EQ:
    Opc = I32 ? Miden::EQ_I32 : Miden::EQ_I64;
    break;
  case ICmpInst::ICMP_NE:
    Opc = I32 ? Miden::NE_I32 : Miden::NE_I64;
    break;
  case ICmpInst::ICMP_UGT:
    Opc = I32 ? Miden::GT_U_I32 : Miden::GT_U_I64;
    break;
  case ICmpInst::ICMP_UGE:
    Opc = I32 ? Miden::GE_U_I32 : Miden::GE_U_I64;
    break;
  case ICmpInst::ICMP_ULT:
    Opc = I32 ? Miden::LT_U_I32 : Miden::LT_U_I64;
    break;
  case ICmpInst::ICMP_ULE:
    Opc = I32 ? Miden::LE_U_I32 : Miden::LE_U_I64;
    break;
  case ICmpInst::ICMP_SGT:
    Opc = I32 ? Miden::GT_S_I32 : Miden::GT_S_I64;
    IsSigned = true;
    break;
  case ICmpInst::ICMP_SGE:
    Opc = I32 ? Miden::GE_S_I32 : Miden::GE_S_I64;
    IsSigned = true;
    break;
  case ICmpInst::ICMP_SLT:
    Opc = I32 ? Miden::LT_S_I32 : Miden::LT_S_I64;
    IsSigned = true;
    break;
  case ICmpInst::ICMP_SLE:
    Opc = I32 ? Miden::LE_S_I32 : Miden::LE_S_I64;
    IsSigned = true;
    break;
  default:
    return false;
  ...
  Register ResultReg = createResultReg(&Miden::I32RegClass);
  BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(Opc), ResultReg)
      .addReg(LHS)
      .addReg(RHS);
...
bool MidenFastISel::selectFCmp(const Instruction *I) {
  ...
  case FCmpInst::FCMP_OEQ:
    Opc = F32 ? Miden::EQ_F32 : Miden::EQ_F64;
    break;
  case FCmpInst::FCMP_UNE:
    Opc = F32 ? Miden::NE_F32 : Miden::NE_F64;
    break;
  case FCmpInst::FCMP_OGT:
    Opc = F32 ? Miden::GT_F32 : Miden::GT_F64;
    break;
  case FCmpInst::FCMP_OGE:
    Opc = F32 ? Miden::GE_F32 : Miden::GE_F64;
    break;
  case FCmpInst::FCMP_OLT:
    Opc = F32 ? Miden::LT_F32 : Miden::LT_F64;
    break;
  case FCmpInst::FCMP_OLE:
    Opc = F32 ? Miden::LE_F32 : Miden::LE_F64;
    break;
...
bool MidenFastISel::selectBitCast(const Instruction *I) {
...
bool MidenFastISel::selectLoad(const Instruction *I) {
  const auto *Load = cast<LoadInst>(I);
  if (Load->isAtomic())
    return false;
  if (!Miden::isDefaultAddressSpace(Load->getPointerAddressSpace()))
    return false;
  ...
  case MVT::i1:
  case MVT::i8:
    Opc = A64 ? Miden::LOAD8_U_I32_A64 : Miden::LOAD8_U_I32_A32;
    RC = &Miden::I32RegClass;
    break;
  case MVT::i16:
    Opc = A64 ? Miden::LOAD16_U_I32_A64 : Miden::LOAD16_U_I32_A32;
    RC = &Miden::I32RegClass;
    break;
  case MVT::i32:
    Opc = A64 ? Miden::LOAD_I32_A64 : Miden::LOAD_I32_A32;
    RC = &Miden::I32RegClass;
    break;
  case MVT::i64:
    Opc = A64 ? Miden::LOAD_I64_A64 : Miden::LOAD_I64_A32;
    RC = &Miden::I64RegClass;
    break;
  case MVT::f32:
    Opc = A64 ? Miden::LOAD_F32_A64 : Miden::LOAD_F32_A32;
    RC = &Miden::F32RegClass;
    break;
  case MVT::f64:
    Opc = A64 ? Miden::LOAD_F64_A64 : Miden::LOAD_F64_A32;
    RC = &Miden::F64RegClass;
    break;
  default:
    return false;
...
bool MidenFastISel::selectStore(const Instruction *I) {
  const auto *Store = cast<StoreInst>(I);
  if (Store->isAtomic())
    return false;
  if (!Miden::isDefaultAddressSpace(Store->getPointerAddressSpace()))
    return false;
  if (!Subtarget->hasSIMD128() &&
  ...
  case MVT::i1:
    ...
    LLVM_FALLTHROUGH;
  case MVT::i8:
    Opc = A64 ? Miden::STORE8_I32_A64 : Miden::STORE8_I32_A32;
    break;
  case MVT::i16:
    Opc = A64 ? Miden::STORE16_I32_A64 : Miden::STORE16_I32_A32;
    break;
  case MVT::i32:
    Opc = A64 ? Miden::STORE_I32_A64 : Miden::STORE_I32_A32;
    break;
  case MVT::i64:
    Opc = A64 ? Miden::STORE_I64_A64 : Miden::STORE_I64_A32;
    break;
  case MVT::f32:
    Opc = A64 ? Miden::STORE_F32_A64 : Miden::STORE_F32_A32;
    break;
  case MVT::f64:
    Opc = A64 ? Miden::STORE_F64_A64 : Miden::STORE_F64_A32;
    break;
  default:
    return false;
  }
...
bool MidenFastISel::selectBr(const Instruction *I) {
...
bool MidenFastISel::selectRet(const Instruction *I) {
  if (!FuncInfo.CanLowerReturn)
    return false;
  ...
  if (Ret->getNumOperands() == 0) {
    BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc, TII.get(Miden::RETURN));
    return true;
  }
...
bool MidenFastISel::selectUnreachable(const Instruction *I) {
  BuildMI(*FuncInfo.MBB, FuncInfo.InsertPt, DbgLoc,
          TII.get(Miden::UNREACHABLE));
  return true;
}

bool MidenFastISel::fastSelectInstruction(const Instruction *I) {
  switch (I->getOpcode()) {
...
FastISel *Miden::createFastISel(FunctionLoweringInfo &FuncInfo,
                                const TargetLibraryInfo *LibInfo) {
  return new MidenFastISel(FuncInfo, LibInfo);
}
