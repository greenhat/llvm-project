//===-- MidenDebugValueManager.cpp - Miden DebugValue Manager -===//
//
...
MidenDebugValueManager::MidenDebugValueManager(
    MachineInstr *Instr) {
...
void MidenDebugValueManager::clone(MachineInstr *Insert,
                                         unsigned NewReg) {
  MachineBasicBlock *MBB = Insert->getParent();
...
void MidenDebugValueManager::replaceWithLocal(unsigned LocalId) {
  for (auto *DBI : DbgValues) {
    auto IndexType = DBI->isIndirectDebugValue()
                         ? llvm::Miden::TI_LOCAL_INDIRECT
                         : llvm::Miden::TI_LOCAL;
    for (auto &MO : DBI->getDebugOperandsForReg(CurrentReg))
