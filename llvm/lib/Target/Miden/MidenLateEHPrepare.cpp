//=== MidenLateEHPrepare.cpp - Miden Exception Preparation -===//
//
...
#define DEBUG_TYPE "miden-late-eh-prepare"

namespace {
class MidenLateEHPrepare final : public MachineFunctionPass {
  StringRef getPassName() const override {
    return "Miden Late Prepare Exception";
  }
...
public:
  ...
  MidenLateEHPrepare() : MachineFunctionPass(ID) {}
};
} // end anonymous namespace

char MidenLateEHPrepare::ID = 0;
INITIALIZE_PASS(MidenLateEHPrepare, DEBUG_TYPE,
                "Miden Late Exception Preparation", false, false)

FunctionPass *llvm::createMidenLateEHPrepare() {
  return new MidenLateEHPrepare();
}
...
MachineBasicBlock *MidenLateEHPrepare::getMatchingEHPad(MachineInstr *MI) {
...
bool MidenLateEHPrepare::runOnMachineFunction(MachineFunction &MF) {
  LLVM_DEBUG(dbgs() << "********** Late EH Prepare **********\n"
                       "********** Function: "
                    << MF.getName() << '\n');

  if (MF.getTarget().getMCAsmInfo()->getExceptionHandlingType() !=
      ExceptionHandling::Miden)
    return false;
...
bool MidenLateEHPrepare::removeUnreachableEHPads(MachineFunction &MF) {
...
// function.
void MidenLateEHPrepare::recordCatchRetBBs(MachineFunction &MF) {
  CatchRetBBs.clear();
  for (auto &MBB : MF) {
    ...
    if (Pos == MBB.end())
      continue;
    MachineInstr *TI = &*Pos;
    if (TI->getOpcode() == Miden::CATCHRET)
...
// bb0:
//   %exn = catch 0
bool MidenLateEHPrepare::hoistCatches(MachineFunction &MF) {
  bool Changed = false;
  SmallVector<MachineInstr *, 16> Catches;
  for (auto &MBB : MF)
    for (auto &MI : MBB)
      if (Miden::isCatch(MI.getOpcode()))
...
bool MidenLateEHPrepare::addCatchAlls(MachineFunction &MF) {
  bool Changed = false;
  ...
  for (auto &MBB : MF) {
    ...
    if (InsertPos == MBB.end() || !Miden::isCatch(InsertPos->getOpcode())) {
      Changed = true;
      BuildMI(MBB, InsertPos,
              InsertPos == MBB.end() ? DebugLoc() : InsertPos->getDebugLoc(),
              TII.get(Miden::CATCH_ALL));
    }
  }
  return Changed;
}
...
// respectively.
bool MidenLateEHPrepare::replaceFuncletReturns(MachineFunction &MF) {
  bool Changed = false;
  const auto &TII = *MF.getSubtarget<MidenSubtarget>().getInstrInfo();
  ...
  for (auto &MBB : MF) {
    ...
    case Miden::CATCHRET: {
      ...
      if (!MBB.isLayoutSuccessor(TBB))
        BuildMI(MBB, TI, TI->getDebugLoc(), TII.get(Miden::BR)).addMBB(TBB);
      TI->eraseFromParent();
      Changed = true;
      break;
    }
    case Miden::CLEANUPRET: {
      // Replace a cleanupret with a rethrow. For C++ support, currently
      // rethrow's immediate argument is always 0 (= the latest exception).
      BuildMI(MBB, TI, TI->getDebugLoc(), TII.get(Miden::RETHROW)).addImm(0);
      TI->eraseFromParent();
      Changed = true;
      break;
    }
...
bool MidenLateEHPrepare::removeUnnecessaryUnreachables(MachineFunction &MF) {
  bool Changed = false;
  for (auto &MBB : MF) {
    for (auto &MI : MBB) {
      if (MI.getOpcode() != Miden::THROW && MI.getOpcode() != Miden::RETHROW)
        continue;
      Changed = true;
...
bool MidenLateEHPrepare::restoreStackPointer(MachineFunction &MF) {
  const auto *FrameLowering = static_cast<const MidenFrameLowering *>(
  ...
  for (auto &MBB : MF) {
    ...
    while (InsertPos != MBB.end() && InsertPos->isEHLabel())
      InsertPos++;
    assert(InsertPos != MBB.end() && Miden::isCatch(InsertPos->getOpcode()) &&
