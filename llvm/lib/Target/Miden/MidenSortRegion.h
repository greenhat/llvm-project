//===-- MidenSortRegion.h - Miden Sort SortRegion ----*- C++-*-===//
//
...
#ifndef LLVM_LIB_TARGET_MIDEN_MIDENSORTREGION_H
#define LLVM_LIB_TARGET_MIDEN_MIDENSORTREGION_H
...
class SortRegionInfo {
  ...
  friend class ConcreteSortRegion<MidenException>;

  const MachineLoopInfo &MLI;
  const MidenExceptionInfo &WEI;
  DenseMap<const MachineLoop *, std::unique_ptr<SortRegion>> LoopMap;
  DenseMap<const MidenException *, std::unique_ptr<SortRegion>> ExceptionMap;

public:
  SortRegionInfo(const MachineLoopInfo &MLI, const MidenExceptionInfo &WEI)
      : MLI(MLI), WEI(WEI) {}
  ...
  // (MachineLoop or MidenException) header. This works when the entity is
  // discontiguous.
  MachineBasicBlock *getBottom(const SortRegion *R);
  MachineBasicBlock *getBottom(const MachineLoop *ML);
  MachineBasicBlock *getBottom(const MidenException *WE);
};
