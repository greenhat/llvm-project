...
using Miden::SortRegion;
using Miden::SortRegionInfo;

#define DEBUG_TYPE "miden-cfg-sort"

// Option to disable EH pad first sorting. Only for testing unwind destination
// mismatches in CFGStackify.
static cl::opt<bool> MidenDisableEHPadSort(
    "miden-disable-ehpad-sort", cl::ReallyHidden,
    cl::desc("Miden: Disable EH pad-first sort order. Testing purpose only."),
    cl::init(false));

namespace {

class MidenCFGSort final : public MachineFunctionPass {
  StringRef getPassName() const override { return "Miden CFG Sort"; }
  ...
  void getAnalysisUsage(AnalysisUsage &AU) const override {
    ...
    AU.addPreserved<MidenExceptionInfo>();
...
public:
  ...
  MidenCFGSort() : MachineFunctionPass(ID) {}
};
} // end anonymous namespace

char MidenCFGSort::ID = 0;
INITIALIZE_PASS(MidenCFGSort, DEBUG_TYPE,
                "Reorders blocks in topological order", false, false)

FunctionPass *llvm::createMidenCFGSort() { return new MidenCFGSort(); }
  ...
  for (MachineBasicBlock *MBB = &MF.front();;) {
    ...
    while (!Preferred.empty()) {
      ...
      if (Next->getNumber() < MBB->getNumber() &&
          (MidenDisableEHPadSort || !Next->isEHPad()) &&
          (!R || !R->contains(Next) ||
...
bool MidenCFGSort::runOnMachineFunction(MachineFunction &MF) {
