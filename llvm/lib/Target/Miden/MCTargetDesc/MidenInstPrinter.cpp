//=- MidenInstPrinter.cpp - Miden assembly instruction printing -=//
//
...
///
/// \file
/// Print MCInst instructions to miden format.
///
//===----------------------------------------------------------------------===//
...
#include "MidenGenAsmWriter.inc"

MidenInstPrinter::MidenInstPrinter(const MCAsmInfo &MAI, const MCInstrInfo &MII,
                                   const MCRegisterInfo &MRI)
    : MCInstPrinter(MAI, MII, MRI) {}

void MidenInstPrinter::printRegName(raw_ostream &OS, unsigned RegNo) const {
  assert(RegNo != MidenFunctionInfo::UnusedReg);
  // Note that there's an implicit local.get/local.set here!
  OS << "$" << RegNo;
}

void MidenInstPrinter::printInst(const MCInst *MI, uint64_t Address,
  ...
  if (Desc.isVariadic()) {
    ...
    for (auto I = Start, E = MI->getNumOperands(); I < E; ++I) {
      if (MI->getOpcode() == Miden::CALL_INDIRECT &&
  ...
  if (CommentStream) {
    ...
    case Miden::CATCH_ALL_S:
      ...
      if (EHInstStack.empty()) {
        ...
        EHInstStack.pop_back();
        if (Opc == Miden::CATCH || Opc == Miden::CATCH_S) {
    ...
    for (unsigned I = 0, E = MI->getNumOperands(); I < E; ++I) {
      ...
      if (I < NumFixedOperands) {
        ...
        if (Desc.OpInfo[I].OperandType != Miden::OPERAND_BASIC_BLOCK)
          continue;
      } else {
        ...
        // when using -miden-keep-registers (in which case they are registers,
...
void MidenInstPrinter::printOperand(const MCInst *MI, unsigned OpNo,
  ...
  if (Op.isReg()) {
    ...
    if (int(WAReg) >= 0)
      ...
      O << "$pop" << MidenFunctionInfo::getWARegStackId(WAReg);
    else if (WAReg != MidenFunctionInfo::UnusedReg)
      O << "$push" << MidenFunctionInfo::getWARegStackId(WAReg);
    else
      O << "$drop";
    ...
    if (SRE->getKind() == MCSymbolRefExpr::VK_MIDEN_TYPEINDEX) {
      auto &Sym = static_cast<const MCSymbolMiden &>(SRE->getSymbol());
      O << Miden::signatureToString(Sym.getSignature());
    } else {
...
void MidenInstPrinter::printBrList(const MCInst *MI, unsigned OpNo,
                                   raw_ostream &O) {
  O << "{";
...
void MidenInstPrinter::printMidenP2AlignOperand(const MCInst *MI, unsigned OpNo,
                                                raw_ostream &O) {
  int64_t Imm = MI->getOperand(OpNo).getImm();
  if (Imm == Miden::GetDefaultP2Align(MI->getOpcode()))
    return;
  O << ":p2align=" << Imm;
}

void MidenInstPrinter::printMidenSignatureOperand(const MCInst *MI,
                                                  unsigned OpNo,
                                                  raw_ostream &O) {
  const MCOperand &Op = MI->getOperand(OpNo);
  if (Op.isImm()) {
    auto Imm = static_cast<unsigned>(Op.getImm());
    if (Imm != miden::MIDEN_TYPE_NORESULT)
      O << Miden::anyTypeToString(Imm);
  } else {
    auto Expr = cast<MCSymbolRefExpr>(Op.getExpr());
    auto *Sym = cast<MCSymbolMiden>(&Expr->getSymbol());
    if (Sym->getSignature()) {
      O << Miden::signatureToString(Sym->getSignature());
    } else {
