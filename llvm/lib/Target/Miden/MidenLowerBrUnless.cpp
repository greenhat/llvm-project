//===-- MidenLowerBrUnless.cpp - Lower br_unless --------------------===//
//
...
#define DEBUG_TYPE "miden-lower-br_unless"

namespace {
class MidenLowerBrUnless final : public MachineFunctionPass {
  StringRef getPassName() const override { return "Miden Lower br_unless"; }
...
public:
  ...
  MidenLowerBrUnless() : MachineFunctionPass(ID) {}
};
} // end anonymous namespace

char MidenLowerBrUnless::ID = 0;
INITIALIZE_PASS(MidenLowerBrUnless, DEBUG_TYPE,
                "Lowers br_unless into inverted br_if", false, false)

FunctionPass *llvm::createMidenLowerBrUnless() {
  return new MidenLowerBrUnless();
}

bool MidenLowerBrUnless::runOnMachineFunction(MachineFunction &MF) {
  ...
  for (auto &MBB : MF) {
    for (MachineInstr &MI : llvm::make_early_inc_range(MBB)) {
      if (MI.getOpcode() != Miden::BR_UNLESS)
        continue;
      ...
      if (!Inverted) {
        Register Tmp = MRI.createVirtualRegister(&Miden::I32RegClass);
        BuildMI(MBB, &MI, MI.getDebugLoc(), TII.get(Miden::EQZ_I32), Tmp)
            .addReg(Cond);
      ...
      assert(Inverted);
      BuildMI(MBB, &MI, MI.getDebugLoc(), TII.get(Miden::BR_IF))
          .add(MI.getOperand(0))
          .addReg(Cond);
      MBB.erase(&MI);
    }
