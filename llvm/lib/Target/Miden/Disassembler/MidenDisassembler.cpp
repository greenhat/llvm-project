//==- MidenDisassembler.cpp - Disassembler for Miden -*- C++ -*-==//
//
...
///
/// \file
/// This file is part of the Miden Disassembler.
///
...
#define DEBUG_TYPE "miden-disassembler"

using DecodeStatus = MCDisassembler::DecodeStatus;

#include "MidenGenDisassemblerTables.inc"

namespace {
static constexpr int MidenInstructionTableSize = 256;

class MidenDisassembler final : public MCDisassembler {
...
public:
  MidenDisassembler(const MCSubtargetInfo &STI, MCContext &Ctx,
                    std::unique_ptr<const MCInstrInfo> MCII)
      : MCDisassembler(STI, Ctx), MCII(std::move(MCII)) {}
};
} // end anonymous namespace

static MCDisassembler *createMidenDisassembler(const Target &T,
                                               const MCSubtargetInfo &STI,
                                               MCContext &Ctx) {
  std::unique_ptr<const MCInstrInfo> MCII(T.createMCInstrInfo());
  return new MidenDisassembler(STI, Ctx, std::move(MCII));
}

extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeMidenDisassembler() {
  // Register the disassembler for each target.
  TargetRegistry::RegisterMCDisassembler(getTheMidenTarget32(),
                                         createMidenDisassembler);
  TargetRegistry::RegisterMCDisassembler(getTheMidenTarget64(),
                                         createMidenDisassembler);
}
...
MidenDisassembler::onSymbolStart(SymbolInfoTy &Symbol, uint64_t &Size,
                                 ArrayRef<uint8_t> Bytes, uint64_t Address,
    ...
    if (LocalEntryCount) {
      ...
      for (int64_t I = 0; I < LocalEntryCount; I++) {
        ...
        for (int64_t J = 0; J < Count; J++) {
          if (I || J)
            outs() << ", ";
          outs() << Miden::anyTypeToString(Type);
        }
      }
    }
  }
...
MidenDisassembler::getInstruction(MCInst &MI, uint64_t &Size,
                                  ArrayRef<uint8_t> Bytes, uint64_t /*Address*/,
  ...
  const auto *MidenInst = &InstructionTable0[Opc];
  // If this is a prefix byte, indirect to another table.
  if (MidenInst->ET == ET_Prefix) {
    MidenInst = nullptr;
    ...
    if (!nextLEB(PrefixedOpc, Bytes, Size, false))
      return MCDisassembler::Fail;
    if (PrefixedOpc < 0 || PrefixedOpc >= MidenInstructionTableSize)
      return MCDisassembler::Fail;
    MidenInst += PrefixedOpc;
  }
  if (MidenInst->ET == ET_Unused)
    return MCDisassembler::Fail;
  // At this point we must have a valid instruction to decode.
  assert(MidenInst->ET == ET_Instruction);
  MI.setOpcode(MidenInst->Opcode);
  // Parse any operands.
  for (uint8_t OPI = 0; OPI < MidenInst->NumOperands; OPI++) {
    auto OT = OperandTable[MidenInst->OperandStart + OPI];
    switch (OT) {
    // ULEB operands:
    case Miden::OPERAND_BASIC_BLOCK:
    case Miden::OPERAND_LOCAL:
    case Miden::OPERAND_GLOBAL:
    case Miden::OPERAND_FUNCTION32:
    case Miden::OPERAND_TABLE:
    case Miden::OPERAND_OFFSET32:
    case Miden::OPERAND_OFFSET64:
    case Miden::OPERAND_P2ALIGN:
    case Miden::OPERAND_TYPEINDEX:
    case Miden::OPERAND_TAG:
    ...
    // SLEB operands:
    case Miden::OPERAND_I32IMM:
    case Miden::OPERAND_I64IMM: {
      if (!parseLEBImmediate(MI, Size, Bytes, true))
        return MCDisassembler::Fail;
      break;
    }
    // block_type operands:
    case Miden::OPERAND_SIGNATURE: {
      int64_t Val;
      ...
      if (Val < 0) {
        ...
        if (Size != PrevSize + 1) {
          MI.addOperand(
              MCOperand::createImm(int64_t(Miden::BlockType::Invalid)));
        } else {
    ...
    case MCOI::OPERAND_REGISTER:
      ...
      // tablegen.
      llvm_unreachable("Register operand in MidenDisassembler");
    default:
      llvm_unreachable("Unknown operand type in MidenDisassembler");
    }
  }
  return MCDisassembler::Success;
}
