//===--- MidenOptimizeLiveIntervals.cpp - LiveInterval processing ---===//
//
...
#define DEBUG_TYPE "miden-optimize-live-intervals"

namespace {
class MidenOptimizeLiveIntervals final : public MachineFunctionPass {
  StringRef getPassName() const override {
    return "Miden Optimize Live Intervals";
  }
...
public:
  ...
  MidenOptimizeLiveIntervals() : MachineFunctionPass(ID) {}
};
} // end anonymous namespace

char MidenOptimizeLiveIntervals::ID = 0;
INITIALIZE_PASS(MidenOptimizeLiveIntervals, DEBUG_TYPE,
                "Optimize LiveIntervals for Miden", false, false)

FunctionPass *llvm::createMidenOptimizeLiveIntervals() {
  return new MidenOptimizeLiveIntervals();
}

bool MidenOptimizeLiveIntervals::runOnMachineFunction(MachineFunction &MF) {
  ...
  for (unsigned I = 0, E = MRI.getNumVirtRegs(); I < E; ++I) {
    ...
    if (Reg == TRI.getFrameRegister(MF) && SplitLIs.size() > 0) {
      ...
      // do.
      MF.getInfo<MidenFunctionInfo>()->setFrameBaseVreg(SplitLIs.back()->reg());
    }
    SplitLIs.clear();
  }
