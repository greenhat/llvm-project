//===-- MidenExplicitLocals.cpp - Make Locals Explicit --------------===//
//
...
///
/// \file
/// This file converts any remaining registers into Miden locals.
///
...
#define DEBUG_TYPE "miden-explicit-locals"

namespace {
class MidenExplicitLocals final : public MachineFunctionPass {
  StringRef getPassName() const override { return "Miden Explicit Locals"; }
...
public:
  ...
  MidenExplicitLocals() : MachineFunctionPass(ID) {}
};
} // end anonymous namespace

char MidenExplicitLocals::ID = 0;
INITIALIZE_PASS(MidenExplicitLocals, DEBUG_TYPE,
                "Convert registers to Miden locals", false, false)

FunctionPass *llvm::createMidenExplicitLocals() {
  return new MidenExplicitLocals();
}

static void checkFrameBase(MidenFunctionInfo &MFI, unsigned Local,
                           unsigned Reg) {
...
static unsigned getLocalId(DenseMap<unsigned, unsigned> &Reg2Local,
                           MidenFunctionInfo &MFI, unsigned &CurLocal,
                           unsigned Reg) {
...
static unsigned getDropOpcode(const TargetRegisterClass *RC) {
  if (RC == &Miden::I32RegClass)
    return Miden::DROP_I32;
  if (RC == &Miden::I64RegClass)
    return Miden::DROP_I64;
  if (RC == &Miden::F32RegClass)
    return Miden::DROP_F32;
  if (RC == &Miden::F64RegClass)
    return Miden::DROP_F64;
  if (RC == &Miden::V128RegClass)
    return Miden::DROP_V128;
  if (RC == &Miden::FUNCREFRegClass)
    return Miden::DROP_FUNCREF;
  if (RC == &Miden::EXTERNREFRegClass)
    return Miden::DROP_EXTERNREF;
...
static unsigned getLocalGetOpcode(const TargetRegisterClass *RC) {
  if (RC == &Miden::I32RegClass)
    return Miden::LOCAL_GET_I32;
  if (RC == &Miden::I64RegClass)
    return Miden::LOCAL_GET_I64;
  if (RC == &Miden::F32RegClass)
    return Miden::LOCAL_GET_F32;
  if (RC == &Miden::F64RegClass)
    return Miden::LOCAL_GET_F64;
  if (RC == &Miden::V128RegClass)
    return Miden::LOCAL_GET_V128;
  if (RC == &Miden::FUNCREFRegClass)
    return Miden::LOCAL_GET_FUNCREF;
  if (RC == &Miden::EXTERNREFRegClass)
    return Miden::LOCAL_GET_EXTERNREF;
  llvm_unreachable("Unexpected register class");
}
...
static unsigned getLocalSetOpcode(const TargetRegisterClass *RC) {
  if (RC == &Miden::I32RegClass)
    return Miden::LOCAL_SET_I32;
  if (RC == &Miden::I64RegClass)
    return Miden::LOCAL_SET_I64;
  if (RC == &Miden::F32RegClass)
    return Miden::LOCAL_SET_F32;
  if (RC == &Miden::F64RegClass)
    return Miden::LOCAL_SET_F64;
  if (RC == &Miden::V128RegClass)
    return Miden::LOCAL_SET_V128;
  if (RC == &Miden::FUNCREFRegClass)
    return Miden::LOCAL_SET_FUNCREF;
  if (RC == &Miden::EXTERNREFRegClass)
    return Miden::LOCAL_SET_EXTERNREF;
  llvm_unreachable("Unexpected register class");
}
...
static unsigned getLocalTeeOpcode(const TargetRegisterClass *RC) {
  if (RC == &Miden::I32RegClass)
    return Miden::LOCAL_TEE_I32;
  if (RC == &Miden::I64RegClass)
    return Miden::LOCAL_TEE_I64;
  if (RC == &Miden::F32RegClass)
    return Miden::LOCAL_TEE_F32;
  if (RC == &Miden::F64RegClass)
    return Miden::LOCAL_TEE_F64;
  if (RC == &Miden::V128RegClass)
    return Miden::LOCAL_TEE_V128;
  if (RC == &Miden::FUNCREFRegClass)
    return Miden::LOCAL_TEE_FUNCREF;
  if (RC == &Miden::EXTERNREFRegClass)
    return Miden::LOCAL_TEE_EXTERNREF;
  llvm_unreachable("Unexpected register class");
}
...
bool MidenExplicitLocals::runOnMachineFunction(MachineFunction &MF) {
  ...
  for (MachineBasicBlock::iterator I = MF.begin()->begin(),
    ...
    MidenDebugValueManager(&MI).replaceWithLocal(Local);
  ...
  for (MachineBasicBlock &MBB : MF) {
    for (MachineInstr &MI : llvm::make_early_inc_range(MBB)) {
      assert(!Miden::isArgument(MI.getOpcode()));

      if (MI.isDebugInstr() || MI.isLabel())
        continue;

      if (MI.getOpcode() == Miden::IMPLICIT_DEF) {
        MI.eraseFromParent();
        Changed = true;
        continue;
      }
      ...
      if (Miden::isTee(MI.getOpcode())) {
        ...
        MidenDebugValueManager(&MI).replaceWithLocal(LocalId);
    ...
    for (const MachineInstr &MI : MBB) {
      ...
      for (const MachineOperand &MO : MI.explicit_operands()) {
        assert((!MO.isReg() || MRI.use_empty(MO.getReg()) ||
               ...
               "MidenExplicitLocals failed to stackify a register operand");
      }
    }
  }
#endif
