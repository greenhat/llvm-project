//===-- MidenCFGStackify.cpp - CFG Stackification -------------------===//
//
...
/// since scope boundaries serve as the labels for Miden's control
/// transfers.
///
/// This is sufficient to convert arbitrary CFGs into a form that works on
/// Miden, provided that all loops are single-entry.
///
/// In case we use exceptions, this pass also fixes mismatches in unwind
/// destinations created during transforming CFG into miden structured format.
///
//===----------------------------------------------------------------------===//

#include "Miden.h"
#include "MidenExceptionInfo.h"
#include "MidenMachineFunctionInfo.h"
#include "MidenSortRegion.h"
...
using Miden::SortRegionInfo;

#define DEBUG_TYPE "miden-cfg-stackify"
...
namespace {
class MidenCFGStackify final : public MachineFunctionPass {
  StringRef getPassName() const override { return "Miden CFG Stackify"; }

  void getAnalysisUsage(AnalysisUsage &AU) const override {
    AU.addRequired<MachineDominatorTree>();
    AU.addRequired<MachineLoopInfo>();
    AU.addRequired<MidenExceptionInfo>();
...
public:
  ...
  MidenCFGStackify() : MachineFunctionPass(ID) {}
  ~MidenCFGStackify() override { releaseMemory(); }
  void releaseMemory() override;
};
} // end anonymous namespace

char MidenCFGStackify::ID = 0;
INITIALIZE_PASS(MidenCFGStackify, DEBUG_TYPE,
                "Insert BLOCK/LOOP/TRY markers for Miden scopes", false, false)

FunctionPass *llvm::createMidenCFGStackify() { return new MidenCFGStackify(); }
...
void MidenCFGStackify::registerScope(MachineInstr *Begin, MachineInstr *End) {
  BeginToEnd[Begin] = End;
  EndToBegin[End] = Begin;
}

// When 'End' is not an 'end_try' but 'delegate, EHPad is nullptr.
void MidenCFGStackify::registerTryScope(MachineInstr *Begin, MachineInstr *End,
                                        MachineBasicBlock *EHPad) {
...
void MidenCFGStackify::unregisterScope(MachineInstr *Begin) {
...
void MidenCFGStackify::placeBlockMarker(MachineBasicBlock &MBB) {
  assert(!MBB.isEHPad());
  ...
  for (const auto &MI : *Header) {
    ...
    // marker.
    if (MI.getOpcode() == Miden::BLOCK || MI.getOpcode() == Miden::TRY) {
    ...
    if (MI.getOpcode() == Miden::END_BLOCK ||
        MI.getOpcode() == Miden::END_LOOP || MI.getOpcode() == Miden::END_TRY)
      BeforeSet.insert(&MI);
#endif
  ...
  for (auto I = Header->getFirstTerminator(), E = Header->begin(); I != E;
    ...
    if (std::prev(I)->isDebugInstr() || std::prev(I)->isPosition())
      continue;
    if (Miden::isChild(*std::prev(I), MFI))
  ...
  // Add the BLOCK.
  Miden::BlockType ReturnType = Miden::BlockType::Void;
  ...
  for (auto &MI : MBB) {
    ...
    if (MI.getOpcode() == Miden::LOOP || MI.getOpcode() == Miden::TRY)
      AfterSet.insert(&MI);
#endif
    ...
    if (MI.getOpcode() == Miden::END_LOOP || MI.getOpcode() == Miden::END_TRY) {
...
void MidenCFGStackify::placeLoopMarker(MachineBasicBlock &MBB) {
  MachineFunction &MF = *MBB.getParent();
  const auto &MLI = getAnalysis<MachineLoopInfo>();
  const auto &WEI = getAnalysis<MidenExceptionInfo>();
  ...
  for (const auto &MI : MBB) {
    ...
    if (MI.getOpcode() == Miden::END_LOOP)
      BeforeSet.insert(&MI);
#ifndef NDEBUG
    else
      AfterSet.insert(&MI);
  ...
  MachineInstr *Begin =
      BuildMI(MBB, InsertPos, MBB.findDebugLoc(InsertPos), TII.get(Miden::LOOP))
          .addImm(int64_t(Miden::BlockType::Void));
  ...
  MachineInstr *End =
      BuildMI(*AfterLoop, InsertPos, EndDL, TII.get(Miden::END_LOOP));
  registerScope(Begin, End);
...
void MidenCFGStackify::placeTryMarker(MachineBasicBlock &MBB) {
  assert(MBB.isEHPad());
  MachineFunction &MF = *MBB.getParent();
  auto &MDT = getAnalysis<MachineDominatorTree>();
  const auto &TII = *MF.getSubtarget<MidenSubtarget>().getInstrInfo();
  ...
  // end.
  MidenException *WE = WEI.getExceptionFor(&MBB);
  assert(WE);
  ...
  if (MBB.isPredecessor(Header)) {
    ...
    if (TermPos == Header->end() || TermPos->getOpcode() != Miden::RETHROW) {
  ...
  for (const auto &MI : *Cont) {
    ...
    if (MI.getOpcode() == Miden::LOOP || MI.getOpcode() == Miden::BLOCK)
      AfterSet.insert(&MI);

    // All END_TRY markers placed earlier belong to exceptions that contains
    // this one.
    if (MI.getOpcode() == Miden::END_TRY)
      AfterSet.insert(&MI);
#endif
...
void MidenCFGStackify::removeUnnecessaryInstrs(MachineFunction &MF) {
  const auto &TII = *MF.getSubtarget<MidenSubtarget>().getInstrInfo();
  ...
  for (auto &MBB : MF) {
    ...
    while (Cont->isEHPad()) {
      ...
      assert(EndTry->getOpcode() != Miden::DELEGATE);
    ...
    if (Analyzable && ((Cond.empty() && TBB && TBB == Cont) ||
      ...
      for (auto I = EHPadLayoutPred->end(), E = EHPadLayoutPred->begin();
        ...
        if (PrevI->isTerminator()) {
          assert(PrevI->getOpcode() == Miden::BR);
  ...
  for (auto &MBB : MF) {
    for (auto &MI : MBB) {
      if (MI.getOpcode() != Miden::TRY)
        continue;
      MachineInstr *Try = &MI, *EndTry = BeginToEnd[Try];
      if (EndTry->getOpcode() == Miden::DELEGATE)
        continue;
      ...
      for (auto B = Try->getIterator(), E = std::next(EndTry->getIterator());
           ...
           std::prev(B)->getOpcode() == Miden::BLOCK &&
           E->getOpcode() == Miden::END_BLOCK &&
...
static void unstackifyVRegsUsedInSplitBB(MachineBasicBlock &MBB,
  ...
  for (MachineInstr &MI : llvm::make_early_inc_range(MBB)) {
    ...
    if (!MFI.isVRegStackified(TeeReg)) {
      ...
      unsigned CopyOpc =
          Miden::getCopyOpcodeForRegClass(MRI.getRegClass(DefReg));
...
void MidenCFGStackify::addTryDelegate(MachineInstr *RangeBegin,
  ...
  MachineInstr *Try =
      BuildMI(*BeginBB, TryPos, RangeBegin->getDebugLoc(), TII.get(Miden::TRY))
          .addImm(int64_t(Miden::BlockType::Void));
  ...
  MachineInstr *Delegate =
      BuildMI(DelegateBB, RangeEnd->getDebugLoc(), TII.get(Miden::DELEGATE))
          .addMBB(DelegateDest);
  registerTryScope(Try, Delegate, nullptr);
}

bool MidenCFGStackify::fixCallUnwindMismatches(MachineFunction &MF) {
  ...
  for (auto &MBB : reverse(MF)) {
    ...
    for (auto &MI : reverse(MBB)) {
      if (MI.getOpcode() == Miden::TRY)
        EHPadStack.pop_back();
      else if (Miden::isCatch(MI.getOpcode()))
      ...
      for (auto *Succ : MBB.successors()) {
        ...
        // details, refer to comments in findMidenUnwindDestinations function in
  ...
  for (auto &MBB : reverse(MF)) {
    bool SeenThrowableInstInBB = false;
    for (auto &MI : reverse(MBB)) {
      bool MayThrow = Miden::mayThrow(MI);
      ...
      // finished a BB.
      else if (RangeEnd && Miden::isMarker(MI.getOpcode()))
...
bool MidenCFGStackify::fixCatchUnwindMismatches(MachineFunction &MF) {
  ...
  const auto *EHInfo = MF.getMidenEHFuncInfo();
  SmallVector<const MachineBasicBlock *, 8> EHPadStack;
  ...
  for (auto &MBB : reverse(MF)) {
    for (auto &MI : reverse(MBB)) {
      if (MI.getOpcode() == Miden::TRY)
        EHPadStack.pop_back();
      else if (MI.getOpcode() == Miden::DELEGATE)
        ...
        // anything
        if (MI.getOpcode() == Miden::CATCH_ALL) {
        }
...
void MidenCFGStackify::recalculateScopeTops(MachineFunction &MF) {
...
/// the function, because it's never reached. However, in Miden, blocks
/// that end at the function end need to have a return type signature that
/// matches the function signature, even though it's unreachable. This function
/// checks for such cases and fixes up the signatures.
void MidenCFGStackify::fixEndsAtEndOfFunction(MachineFunction &MF) {
  const auto &MFI = *MF.getInfo<MidenFunctionInfo>();
  ...
  Miden::BlockType RetType =
      MFI.getResults().size() > 1
          ? Miden::BlockType::Multivalue
          : Miden::BlockType(Miden::toValType(MFI.getResults().front()));
  ...
  auto Process = [&](MachineBasicBlock::reverse_iterator It) {
    ...
    while (It != MBB->rend()) {
      ...
      case Miden::END_TRY: {
        ...
        assert(EHPad);
        auto NextIt = std::next(Miden::findCatch(EHPad)->getReverseIterator());
...
// Miden functions end with an end instruction, as if the function body
// were a block.
static void appendEndToFunction(MachineFunction &MF,
  ...
  BuildMI(MF.back(), MF.back().end(),
          ...
          TII.get(Miden::END_FUNCTION));
}

/// Insert LOOP/TRY/BLOCK markers at appropriate places.
void MidenCFGStackify::placeMarkers(MachineFunction &MF) {
  ...
  for (auto &MBB : MF) {
    if (MBB.isEHPad()) {
      ...
      if (MCAI->getExceptionHandlingType() == ExceptionHandling::Miden &&
...
unsigned
MidenCFGStackify::getBranchDepth(const SmallVectorImpl<EndMarkerInfo> &Stack,
                                 const MachineBasicBlock *MBB) {
  unsigned Depth = 0;
...
unsigned
MidenCFGStackify::getDelegateDepth(const SmallVectorImpl<EndMarkerInfo> &Stack,
                                   const MachineBasicBlock *MBB) {
  if (MBB == FakeCallerBB)
    return Stack.size();
...
unsigned MidenCFGStackify::getRethrowDepth(
  ...
  for (auto X : reverse(Stack)) {
    ...
    if (End->getOpcode() == Miden::END_TRY) {
...
void MidenCFGStackify::rewriteDepthImmediates(MachineFunction &MF) {
    ...
    for (MachineInstr &MI : llvm::reverse(MBB)) {
      ...
      default:
        if (MI.isTerminator()) {
          ...
          for (auto MO : Ops) {
            if (MO.isMBB()) {
              if (MI.getOpcode() == Miden::DELEGATE)
...
void MidenCFGStackify::cleanupFunctionData(MachineFunction &MF) {
  if (FakeCallerBB)
    MF.deleteMachineBasicBlock(FakeCallerBB);
  AppendixBB = FakeCallerBB = nullptr;
}

void MidenCFGStackify::releaseMemory() {
  ScopeTops.clear();
  BeginToEnd.clear();
  EndToBegin.clear();
  TryToEHPad.clear();
...
bool MidenCFGStackify::runOnMachineFunction(MachineFunction &MF) {
  ...
  const auto &TII = *MF.getSubtarget<MidenSubtarget>().getInstrInfo();
  if (!MF.getSubtarget<MidenSubtarget>().getTargetTriple().isOSBinFormatELF())
    appendEndToFunction(MF, TII);

  cleanupFunctionData(MF);

  MF.getInfo<MidenFunctionInfo>()->setCFGStackified();
  return true;
}
