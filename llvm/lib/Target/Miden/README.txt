//===-- README.txt - Notes for Miden code gen -----------------------===//

The object format emitted by the Miden backed is documented in:

  * https://github.com/Miden/tool-conventions/blob/main/Linking.md

The C ABI is described in:

  * https://github.com/Miden/tool-conventions/blob/main/BasicCABI.md

For more information on Miden itself, see the home page:

  * https://webassembly.github.io/

Emscripten provides a C/C++ compilation environment based on clang which
includes standard libraries, tools, and packaging for producing Miden
...
Rust provides Miden support integrated into Cargo. There are two
main options:
 - miden32-unknown-unknown, which provides a relatively minimal environment
   that has an emphasis on being "native"
 - miden32-unknown-emscripten, which uses Emscripten internally and
...
encoding of Miden itself:
  * https://github.com/Miden/design/blob/main/Semantics.md
  * https://github.com/Miden/design/blob/main/BinaryEncoding.md
...
Would these be useful to run for Miden too? Also, it has an option to
...
Lots of things could be done in MidenTargetTransformInfo.cpp. Similarly,
...
let the MidenPeephole pass clean up dead defs for such calls, as it does
for stores.
...
being run after the Miden PEI pass.
...
Miden registers are implicitly initialized to zero. Explicit zeroing is
therefore often redundant and could be optimized away.

//===---------------------------------------------------------------------===//

Small indices may use smaller encodings than large indices.
MidenRegColoring and/or MidenRegRenumbering should sort registers
...
than via the transform in MidenFixIrreducibleControlFlow.cpp.
...
MidenRegStackify could use AliasAnalysis to reorder loads and stores more
aggressively.

//===---------------------------------------------------------------------===//

MidenRegStackify is currently a greedy algorithm. This means that, for
...
MidenRegStackify currently assumes that the stack must be empty after
an instruction with no return values, however miden doesn't actually require
this. MidenRegStackify could be extended, or possibly rewritten, to take
full advantage of what Miden permits.

//===---------------------------------------------------------------------===//

Add support for mergeable sections in the Miden writer, such as for strings and
floating-point constants.

//===---------------------------------------------------------------------===//

The function @dynamic_alloca_redzone in test/CodeGen/Miden/userstack.ll
