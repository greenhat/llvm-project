//===-- MidenRegNumbering.cpp - Register Numbering ------------------===//
//
...
///
/// \file
/// This file implements a pass which assigns Miden register
...
#define DEBUG_TYPE "miden-reg-numbering"

namespace {
class MidenRegNumbering final : public MachineFunctionPass {
  StringRef getPassName() const override { return "Miden Register Numbering"; }
...
public:
  ...
  MidenRegNumbering() : MachineFunctionPass(ID) {}
};
} // end anonymous namespace

char MidenRegNumbering::ID = 0;
INITIALIZE_PASS(MidenRegNumbering, DEBUG_TYPE,
                "Assigns Miden register numbers for virtual registers", false,
                false)

FunctionPass *llvm::createMidenRegNumbering() {
  return new MidenRegNumbering();
}

bool MidenRegNumbering::runOnMachineFunction(MachineFunction &MF) {
  ...
  // Miden argument registers are in the same index space as local
  ...
  // Then assign regular Miden registers for all remaining used
  ...
  for (unsigned VRegIdx = 0; VRegIdx < NumVRegs; ++VRegIdx) {
    ...
    if (MFI.getWAReg(VReg) == MidenFunctionInfo::UnusedReg) {
