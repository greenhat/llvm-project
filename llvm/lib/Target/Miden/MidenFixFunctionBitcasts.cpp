//===-- MidenFixFunctionBitcasts.cpp - Fix function bitcasts --------===//
//
...
///
/// Miden requires caller and callee signatures to match, however in LLVM,
...
#define DEBUG_TYPE "miden-fix-function-bitcasts"

namespace {
class FixFunctionBitcasts final : public ModulePass {
  StringRef getPassName() const override {
    return "Miden Fix Function Bitcasts";
  }
...
INITIALIZE_PASS(FixFunctionBitcasts, DEBUG_TYPE,
                "Fix mismatching bitcasts for Miden", false, false)

ModulePass *llvm::createMidenFixFunctionBitcasts() {
...
// instead).
//
// If there is a type mismatch that we know would result in an invalid miden
...
// would be equivalent at the miden level and so we can't know if we need to
// generate a wrapper.
