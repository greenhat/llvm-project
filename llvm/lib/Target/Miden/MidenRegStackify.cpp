//===-- MidenRegStackify.cpp - Register Stackification --------------===//
//
...
#include "MCTargetDesc/MidenMCTargetDesc.h" // for Miden::ARGUMENT_*
#include "Miden.h"
#include "MidenDebugValueManager.h"
...
#define DEBUG_TYPE "miden-reg-stackify"

namespace {
class MidenRegStackify final : public MachineFunctionPass {
  StringRef getPassName() const override { return "Miden Register Stackify"; }
...
public:
  ...
  MidenRegStackify() : MachineFunctionPass(ID) {}
};
} // end anonymous namespace

char MidenRegStackify::ID = 0;
INITIALIZE_PASS(MidenRegStackify, DEBUG_TYPE,
                "Reorder instructions to use the Miden value stack", false,
                false)

FunctionPass *llvm::createMidenRegStackify() { return new MidenRegStackify(); }
...
static void imposeStackOrdering(MachineInstr *MI) {
  ...
  if (!MI->definesRegister(Miden::VALUE_STACK))
    MI->addOperand(MachineOperand::CreateReg(Miden::VALUE_STACK,
                                             /*isDef=*/true,
                                             /*isImp=*/true));

  // Also read the opaque VALUE_STACK register.
  if (!MI->readsRegister(Miden::VALUE_STACK))
    MI->addOperand(MachineOperand::CreateReg(Miden::VALUE_STACK,
                                             /*isDef=*/false,
                                             /*isImp=*/true));
}
...
static void convertImplicitDefToConstZero(MachineInstr *MI,
  ...
  if (RegClass == &Miden::I32RegClass) {
    MI->setDesc(TII->get(Miden::CONST_I32));
    MI->addOperand(MachineOperand::CreateImm(0));
  } else if (RegClass == &Miden::I64RegClass) {
    MI->setDesc(TII->get(Miden::CONST_I64));
    MI->addOperand(MachineOperand::CreateImm(0));
  } else if (RegClass == &Miden::F32RegClass) {
    MI->setDesc(TII->get(Miden::CONST_F32));
    auto *Val = cast<ConstantFP>(Constant::getNullValue(
        Type::getFloatTy(MF.getFunction().getContext())));
    MI->addOperand(MachineOperand::CreateFPImm(Val));
  } else if (RegClass == &Miden::F64RegClass) {
    MI->setDesc(TII->get(Miden::CONST_F64));
    auto *Val = cast<ConstantFP>(Constant::getNullValue(
        Type::getDoubleTy(MF.getFunction().getContext())));
    MI->addOperand(MachineOperand::CreateFPImm(Val));
  } else if (RegClass == &Miden::V128RegClass) {
    MI->setDesc(TII->get(Miden::CONST_V128_I64x2));
    MI->addOperand(MachineOperand::CreateImm(0));
    MI->addOperand(MachineOperand::CreateImm(0));
  } else {
...
static void queryCallee(const MachineInstr &MI, bool &Read, bool &Write,
  ...
  const MachineOperand &MO = Miden::getCalleeOp(MI);
  if (MO.isGlobal()) {
...
static void query(const MachineInstr &MI, bool &Read, bool &Write,
  ...
  if (MI.mayStore()) {
    ...
    switch (MI.getOpcode()) {
    case Miden::DIV_S_I32:
    case Miden::DIV_S_I64:
    case Miden::REM_S_I32:
    case Miden::REM_S_I64:
    case Miden::DIV_U_I32:
    case Miden::DIV_U_I64:
    case Miden::REM_U_I32:
    case Miden::REM_U_I64:
    case Miden::I32_TRUNC_S_F32:
    case Miden::I64_TRUNC_S_F32:
    case Miden::I32_TRUNC_S_F64:
    case Miden::I64_TRUNC_S_F64:
    case Miden::I32_TRUNC_U_F32:
    case Miden::I64_TRUNC_U_F32:
    case Miden::I32_TRUNC_U_F64:
    case Miden::I64_TRUNC_U_F64:
  ...
  if ((MI.getOpcode() == Miden::GLOBAL_SET_I32 ||
       MI.getOpcode() == Miden::GLOBAL_SET_I64) &&
...
static bool isSafeToMove(const MachineOperand *Def, const MachineOperand *Use,
  ...
  // move.
  if (Miden::isCatch(DefI->getOpcode()))
    return false;
...
static unsigned getTeeOpcode(const TargetRegisterClass *RC) {
  if (RC == &Miden::I32RegClass)
    return Miden::TEE_I32;
  if (RC == &Miden::I64RegClass)
    return Miden::TEE_I64;
  if (RC == &Miden::F32RegClass)
    return Miden::TEE_F32;
  if (RC == &Miden::F64RegClass)
    return Miden::TEE_F64;
  if (RC == &Miden::V128RegClass)
    return Miden::TEE_V128;
  if (RC == &Miden::EXTERNREFRegClass)
    return Miden::TEE_EXTERNREF;
  if (RC == &Miden::FUNCREFRegClass)
    return Miden::TEE_FUNCREF;
...
static MachineInstr *moveForSingleUse(unsigned Reg, MachineOperand &Op,
  ...
  MidenDebugValueManager DefDIs(Def);
...
static MachineInstr *rematerializeCheapDef(
    ...
    MidenFunctionInfo &MFI, MachineRegisterInfo &MRI, const MidenInstrInfo *TII,
    const MidenRegisterInfo *TRI) {
  ...
  if (IsDead) {
    ...
    LIS.removePhysRegDefAt(MCRegister::from(Miden::ARGUMENTS), Idx);
...
/// resulting code.
static MachineInstr *
moveAndTeeForMultiUse(unsigned Reg, MachineOperand &Op, MachineInstr *Def,
                      MachineBasicBlock &MBB, MachineInstr *Insert,
                      LiveIntervals &LIS, MidenFunctionInfo &MFI,
                      MachineRegisterInfo &MRI, const MidenInstrInfo *TII) {
...
bool MidenRegStackify::runOnMachineFunction(MachineFunction &MF) {
  ...
  const auto *TRI = MF.getSubtarget<MidenSubtarget>().getRegisterInfo();
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    for (auto MII = MBB.rbegin(); MII != MBB.rend(); ++MII) {
      ...
      while (!TreeWalker.done()) {
        ...
        // instructions.
        if (Miden::isArgument(DefI->getOpcode()))
          continue;
  ...
  if (Changed) {
    MF.getRegInfo().addLiveIn(Miden::VALUE_STACK);
    for (MachineBasicBlock &MBB : MF)
      MBB.addLiveIn(Miden::VALUE_STACK);
  }
