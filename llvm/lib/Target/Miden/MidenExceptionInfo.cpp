//===--- MidenExceptionInfo.cpp - Exception Infomation --------------===//
//
...
#define DEBUG_TYPE "miden-exception-info"

char MidenExceptionInfo::ID = 0;

INITIALIZE_PASS_BEGIN(MidenExceptionInfo, DEBUG_TYPE,
                      "Miden Exception Information", true, true)
INITIALIZE_PASS_DEPENDENCY(MachineDominatorTree)
INITIALIZE_PASS_DEPENDENCY(MachineDominanceFrontier)
INITIALIZE_PASS_END(MidenExceptionInfo, DEBUG_TYPE,
                    "Miden Exception Information", true, true)

bool MidenExceptionInfo::runOnMachineFunction(MachineFunction &MF) {
...
void MidenExceptionInfo::recalculate(MachineFunction &MF,
                                     MachineDominatorTree &MDT,
                                     const MachineDominanceFrontier &MDF) {
  // Postorder traversal of the dominator tree.
  SmallVector<std::unique_ptr<MidenException>, 8> Exceptions;
  for (auto DomNode : post_order(&MDT)) {
    MachineBasicBlock *EHPad = DomNode->getBlock();
    if (!EHPad->isEHPad())
      continue;
    auto WE = std::make_unique<MidenException>(EHPad);
    discoverAndMapException(WE.get(), MDT, MDF);
    Exceptions.push_back(std::move(WE));
  }

  // MidenEHFuncInfo contains a map of <catchpad, its next unwind destination>,
  ...
  SmallVector<std::pair<MidenException *, MidenException *>> UnwindWEVec;
  ...
  for (auto *DomNode : post_order(&MDT)) {
    ...
    MidenException *WE = getExceptionFor(MBB);
    ...
    for (auto *MBB : SrcWE->getBlocksSet()) {
      ...
      if (isReachableAmongDominated(DstWE->getEHPad(), MBB, SrcWE->getEHPad(),
        ...
        LLVM_DEBUG(dbgs() << "Remainder BB: " << MBB->getNumber() << "."
                          << MBB->getName() << " is\n");
        MidenException *InnerWE = getExceptionFor(MBB);
        while (InnerWE != SrcWE) {
          LLVM_DEBUG(dbgs()
  ...
  SmallVector<MidenException *, 8> ExceptionPointers;
...
void MidenExceptionInfo::releaseMemory() {
  BBMap.clear();
  TopLevelExceptions.clear();
}

void MidenExceptionInfo::getAnalysisUsage(AnalysisUsage &AU) const {
  AU.setPreservesAll();
...
void MidenExceptionInfo::discoverAndMapException(
    MidenException *WE, const MachineDominatorTree &MDT,
    const MachineDominanceFrontier &MDF) {
  ...
  while (!WL.empty()) {
    ...
    MidenException *SubE = getOutermostException(MBB);
    if (SubE) {
      if (SubE != WE) {
...
MidenException *
MidenExceptionInfo::getOutermostException(MachineBasicBlock *MBB) const {
  MidenException *WE = getExceptionFor(MBB);
  if (WE) {
    while (MidenException *Parent = WE->getParentException())
      WE = Parent;
  }
  return WE;
}

void MidenException::print(raw_ostream &OS, unsigned Depth) const {
...
LLVM_DUMP_METHOD void MidenException::dump() const { print(dbgs()); }
#endif
...
void MidenExceptionInfo::print(raw_ostream &OS, const Module *) const {
