// MidenMachineFunctionInfo.h-Miden machine function info-*- C++ -*-
//
...
///
/// \file
/// This file declares Miden-specific per-machine-function
/// information.
///
//===----------------------------------------------------------------------===//

#ifndef LLVM_LIB_TARGET_MIDEN_MIDENMACHINEFUNCTIONINFO_H
#define LLVM_LIB_TARGET_MIDEN_MIDENMACHINEFUNCTIONINFO_H
...
namespace yaml {
struct MidenFunctionInfo;
}

/// This class is derived from MachineFunctionInfo and contains private
/// Miden-specific information for each MachineFunction.
class MidenFunctionInfo final : public MachineFunctionInfo {
  ...
  /// A mapping from CodeGen vreg index to Miden register number.
  ...
  // after MidenExplicitLocals
  unsigned FrameBaseLocal = -1U;

  // Function properties.
  bool CFGStackified = false;

  // Catchpad unwind destination info for miden EH.
  MidenEHFuncInfo *MidenEHInfo = nullptr;

public:
  explicit MidenFunctionInfo(MachineFunction &MF_)
      : MF(&MF_), MidenEHInfo(MF_.getMidenEHFuncInfo()) {}
  ~MidenFunctionInfo() override;
  ...
  void initializeBaseYamlFields(const yaml::MidenFunctionInfo &YamlMFI);
  ...
  MidenEHFuncInfo *getMidenEHFuncInfo() const { return MidenEHInfo; }
  void setMidenEHFuncInfo(MidenEHFuncInfo *Info) { MidenEHInfo = Info; }
};

void computeLegalValueVTs(const MidenTargetLowering &TLI, LLVMContext &Ctx,
...
struct MidenFunctionInfo final : public yaml::MachineFunctionInfo {
  std::vector<FlowStringValue> Params;
  std::vector<FlowStringValue> Results;
  bool CFGStackified = false;
  // The same as MidenEHFuncInfo's SrcToUnwindDest, but stored in the mapping of
  // BB numbers
  BBNumberMap SrcToUnwindDest;

  MidenFunctionInfo() = default;
  MidenFunctionInfo(const llvm::MidenFunctionInfo &MFI);

  void mappingImpl(yaml::IO &YamlIO) override;
  ~MidenFunctionInfo() = default;
};

template <> struct MappingTraits<MidenFunctionInfo> {
  static void mapping(IO &YamlIO, MidenFunctionInfo &MFI) {
    ...
    YamlIO.mapOptional("midenEHFuncInfo", MFI.SrcToUnwindDest);
  }
};
