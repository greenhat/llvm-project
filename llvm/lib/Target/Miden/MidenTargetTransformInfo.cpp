//===-- MidenTargetTransformInfo.cpp - Miden-specific TTI -----===//
//
...
///
/// \file
/// This file defines the Miden-specific TargetTransformInfo
/// implementation.
///
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "midentti"

TargetTransformInfo::PopcntSupportKind
MidenTTIImpl::getPopcntSupport(unsigned TyWidth) const {
  assert(isPowerOf2_32(TyWidth) && "Ty width must be power of 2");
  return TargetTransformInfo::PSK_FastHardware;
}

unsigned MidenTTIImpl::getNumberOfRegisters(unsigned ClassID) const {
...
TypeSize
MidenTTIImpl::getRegisterBitWidth(TargetTransformInfo::RegisterKind K) const {
  switch (K) {
...
InstructionCost MidenTTIImpl::getArithmeticInstrCost(
  ...
  InstructionCost Cost = BasicTTIImplBase<MidenTTIImpl>::getArithmeticInstrCost(
...
InstructionCost MidenTTIImpl::getVectorInstrCost(unsigned Opcode, Type *Val,
                                                 unsigned Index) {
...
bool MidenTTIImpl::areInlineCompatible(const Function *Caller,
                                       const Function *Callee) const {
  // Allow inlining only when the Callee has a subset of the Caller's
  // features. In principle, we should be able to inline regardless of any
  // features because Miden supports features at module granularity, not
...
void MidenTTIImpl::getUnrollingPreferences(
...
bool MidenTTIImpl::supportsTailCalls() const { return getST()->hasTailCall(); }
