//=== MidenLowerEmscriptenEHSjLj.cpp - Lower exceptions for Emscripten =//
//
...
/// and catch mechanism in case of Emscripten EH/SjLj and Miden EH intrinsics in
...
/// miden code and handle it appropriately.
///
...
/// we can check / whether a longjmp occurred from miden code. Each block with a
...
///    signature. After LLVM IR types are lowered to the target miden types,
///    the names for these wrappers will change based on miden types as well,
...
///    label.
///
/// * Miden setjmp / longjmp handling
/// This mode still uses some Emscripten library functions but not JavaScript's
/// try-catch mechanism. It instead uses Miden exception handling intrinsics,
...
/// transformation)
///
/// 4) Create a catchpad with a miden.catch() intrinsic, which returns the value
/// thrown by __miden_longjmp function. In Emscripten library, we have this
/// struct:
///
/// struct __MidenLongjmpArgs {
///   void *env;
///   int val;
/// };
/// struct __MidenLongjmpArgs __miden_longjmp_args;
///
/// The thrown value here is a pointer to __miden_longjmp_args struct object. We
/// use this struct to transfer two values by throwing a single value. Miden
...
/// Miden SjLj we do the testing in only one place, in this catchpad.
///
...
/// by calling __miden_longjmp(). If it corresponds to one of setjmps in the
...
/// every longjmpable callsite; in Miden SjLj we do this only once at the top of
...
///
/// catch.longjmp:
///   %longjmp.args = miden.catch() ;; struct __MidenLongjmpArgs
///   %env = load 'env' field from __MidenLongjmpArgs
///   %val = load 'val' field from __MidenLongjmpArgs
...
#define DEBUG_TYPE "miden-lower-em-ehsjlj"
...
namespace {
class MidenLowerEmscriptenEHSjLj final : public ModulePass {
  bool EnableEmEH;      // Enable Emscripten exception handling
  bool EnableEmSjLj;    // Enable Emscripten setjmp/longjmp handling
  bool EnableMidenSjLj; // Enable Miden setjmp/longjmp handling
  ...
  Function *MidenLongjmpF = nullptr;      // __miden_longjmp() (Emscripten)
  Function *CatchF = nullptr;             // miden.catch() (intrinsic)

  // type of 'struct __MidenLongjmpArgs' defined in emscripten
  ...
  StringRef getPassName() const override {
    return "Miden Lower Emscripten Exceptions";
  }
  ...
  void
  handleLongjmpableCallsForMidenSjLj(Function &F, InstVector &SetjmpTableInsts,
                                     InstVector &SetjmpTableSizeInsts,
                                     SmallVectorImpl<PHINode *> &SetjmpRetPHIs);
...
public:
  ...
  MidenLowerEmscriptenEHSjLj()
      : ModulePass(ID), EnableEmEH(Miden::MidenEnableEmEH),
        EnableEmSjLj(Miden::MidenEnableEmSjLj),
        EnableMidenSjLj(Miden::MidenEnableSjLj) {
    assert(!(EnableEmSjLj && EnableMidenSjLj) &&
           "Two SjLj modes cannot be turned on at the same time");
    assert(!(EnableEmEH && EnableMidenSjLj) &&
           "Miden SjLj should be only used with Miden EH");
...
char MidenLowerEmscriptenEHSjLj::ID = 0;
INITIALIZE_PASS(MidenLowerEmscriptenEHSjLj, DEBUG_TYPE,
                "Miden Lower Emscripten Exceptions / Setjmp / Longjmp", false,
                false)

ModulePass *llvm::createMidenLowerEmscriptenEHSjLj() {
  return new MidenLowerEmscriptenEHSjLj();
}
...
static std::string getSignature(FunctionType *FTy) {
  ...
  // When s2miden parses .s file, a comma means the end of an argument. So a
...
static Function *getEmscriptenFunction(FunctionType *Ty, const Twine &Name,
  ...
  if (!F->hasFnAttribute("miden-import-module")) {
    ...
    B.addAttribute("miden-import-module", "env");
    F->addFnAttrs(B);
  }
  if (!F->hasFnAttribute("miden-import-name")) {
    llvm::AttrBuilder B(M->getContext());
    B.addAttribute("miden-import-name", F->getName());
    F->addFnAttrs(B);
  }
  return F;
}
...
// i32 for miden32 and i64 for miden64.
...
// space. Returns (i32 C) for miden32 and (i64 C) for miden64, when C is the
// integer.
...
// instruction.
Function *
MidenLowerEmscriptenEHSjLj::getFindMatchingCatch(Module &M,
                                                 unsigned NumClauses) {
...
Value *MidenLowerEmscriptenEHSjLj::wrapInvoke(CallBase *CI) {
  ...
  NewCall->takeName(CI);
  // NewCall->setCallingConv(CallingConv::MIDEN_EmscriptenInvoke);
...
Function *MidenLowerEmscriptenEHSjLj::getInvokeWrapper(CallBase *CI) {
  Module *M = CI->getModule();
...
static bool canLongjmp(const Value *Callee) {
  ...
  //
  // We intentionally treat __cxa_end_catch longjmpable in Miden SjLj even
  // though it surely cannot longjmp, in order to maintain the unwind
  // relationship from all existing catchpads (and calls within them) to
  // catch.dispatch.longjmp.
  //
  // In Miden EH + Miden SjLj, we
...
void MidenLowerEmscriptenEHSjLj::wrapTestSetjmp(
...
void MidenLowerEmscriptenEHSjLj::rebuildSSA(Function &F) {
...
// In Miden SjLj, the new function is
//   void __miden_longjmp(i8*, i32)
...
// eventually be lowered to i32/i64 in the miden backend.
void MidenLowerEmscriptenEHSjLj::replaceLongjmpWith(Function *LongjmpF,
                                                    Function *NewF) {
  assert(NewF == EmLongjmpF || NewF == MidenLongjmpF);
  Module *M = LongjmpF->getParent();
  SmallVector<CallInst *, 8> ToErase;
  LLVMContext &C = LongjmpF->getParent()->getContext();
  IRBuilder<> IRB(C);

  // For calls to longjmp, replace it with emscripten_longjmp/__miden_longjmp
  // and cast its first argument (jmp_buf*) appropriately
  ...
  // with (void(*)(jmp_buf*, int))emscripten_longjmp / __miden_longjmp.
...
bool MidenLowerEmscriptenEHSjLj::runOnModule(Module &M) {
  ...
  auto &TM = TPC->getTM<MidenTargetMachine>();
  ...
  if ((EnableEmSjLj || EnableMidenSjLj) && SetjmpF) {
  ...
  DoSjLj = (EnableEmSjLj | EnableMidenSjLj) && (SetjmpUsed || LongjmpUsed);
  ...
  if (DoSjLj) {
    ...
    } else { // EnableMidenSjLj
      // Register __miden_longjmp function, which calls __builtin_miden_longjmp.
      FunctionType *FTy = FunctionType::get(
          IRB.getVoidTy(), {IRB.getInt8PtrTy(), IRB.getInt32Ty()}, false);
      MidenLongjmpF = getEmscriptenFunction(FTy, "__miden_longjmp", &M);
      MidenLongjmpF->addFnAttr(Attribute::NoReturn);
    }
    ...
    if (SetjmpF) {
      ...
      // miden.catch() will be lowered down to miden 'catch' instruction in
      // instruction selection.
      CatchF = Intrinsic::getDeclaration(&M, Intrinsic::miden_catch);
      // Type for struct __MidenLongjmpArgs
  ...
  if (DoSjLj) {
    ...
    if (LongjmpF)
      replaceLongjmpWith(LongjmpF, EnableEmSjLj ? EmLongjmpF : MidenLongjmpF);
  ...
  if ((EnableEmSjLj || EnableMidenSjLj) && !SetjmpUsersToNullify.empty()) {
    Changed = true;
    assert(SetjmpF);
  ...
  for (auto *V : {GetTempRet0F, SetTempRet0F, ResumeF, EHTypeIDF, EmLongjmpF,
                  SaveSetjmpF, TestSetjmpF, MidenLongjmpF, CatchF})
    if (V && V->use_empty())
      V->eraseFromParent();
...
bool MidenLowerEmscriptenEHSjLj::runEHOnFunction(Function &F) {
  Module &M = *F.getParent();
...
bool MidenLowerEmscriptenEHSjLj::runSjLjOnFunction(Function &F) {
  assert(EnableEmSjLj || EnableMidenSjLj);
  Module &M = *F.getParent();
  LLVMContext &C = F.getContext();
  IRBuilder<> IRB(C);
  ...
  for (auto *U : make_early_inc_range(SetjmpF->users())) {
    ...
    if (CB->getOperandBundle(LLVMContext::OB_funclet)) {
      ...
      SS << "In function " + F.getName() +
                ": setjmp within a catch clause is not supported in Miden "
                "EH:\n";
      SS << *CB;
  ...
  if (EnableEmSjLj)
    ...
    handleLongjmpableCallsForMidenSjLj(F, SetjmpTableInsts,
  ...
  for (BasicBlock &BB : F) {
    ...
    // happens if we use Miden EH or Miden SjLJ.
    for (auto &I : BB) {
...
void MidenLowerEmscriptenEHSjLj::handleLongjmpableCallsForEmscriptenSjLj(
  ...
  for (unsigned I = 0; I < BBs.size(); I++) {
    ...
    for (Instruction &I : *BB) {
      if (isa<InvokeInst>(&I)) {
        ...
        raw_string_ostream SS(S);
        SS << "In function " << F.getName()
           << ": When using Miden EH with Emscripten SjLj, there is a "
...
// BBs. Refer to 4) of "Miden setjmp/longjmp handling" section in the comments
// at top of the file for details.
void MidenLowerEmscriptenEHSjLj::handleLongjmpableCallsForMidenSjLj(
    Function &F, InstVector &SetjmpTableInsts, InstVector &SetjmpTableSizeInsts,
    SmallVectorImpl<PHINode *> &SetjmpRetPHIs) {
  Module &M = *F.getParent();
  LLVMContext &C = F.getContext();
  ...
  // function attached to it. Search for the miden personality function, and if
  // it exists, use it, and if it doesn't, create a dummy personality function.
  // (SjLj is not going to call it anyway.)
  // if (!F.hasPersonalityFn()) {
  //   StringRef PersName = getEHPersonalityName(EHPersonality::Miden_CXX);
  ...
  // Miden throw and catch instructions can throw and catch multiple values, but
  ...
  // type 'struct __MidenLongjmpArgs', which is defined in Emscripten.
  Instruction *CatchCI =
      IRB.CreateCall(CatchF, {IRB.getInt32(Miden::C_LONGJMP)}, "thrown");
  Value *LongjmpArgs =
  ...
  // void *env = __miden_longjmp_args.env;
  Instruction *Env = IRB.CreateLoad(IRB.getInt8PtrTy(), EnvField, "env");
  // int val = __miden_longjmp_args.val;
  ...
  CallInst *MidenLongjmpCI = IRB.CreateCall(
      MidenLongjmpF, {Env, Val}, OperandBundleDef("funclet", CatchPad));
  IRB.CreateUnreachable();
    ...
    for (auto &I : *BB) {
      ...
      if (isEmAsmCall(Callee))
        report_fatal_error("Cannot use EM_ASM* alongside setjmp/longjmp in " +
                               ...
                               "EM_ASM into another function.",
                           false);
      // This is __miden_longjmp() call we inserted in this function, which
  ...
  for (auto *CI : LongjmpableCalls) {
    ...
    // all C functions, it can longjmp, which means it can throw a Miden
    // exception now.
