//=- MidenFixBrTableDefaults.cpp - Fix br_table default branch targets -//
//
...
#define DEBUG_TYPE "miden-fix-br-table-defaults"

namespace {

class MidenFixBrTableDefaults final : public MachineFunctionPass {
  StringRef getPassName() const override {
    return "Miden Fix br_table Defaults";
  }
...
public:
  ...
  MidenFixBrTableDefaults() : MachineFunctionPass(ID) {}
};

char MidenFixBrTableDefaults::ID = 0;

// Target indepedent selection dag assumes that it is ok to use PointerTy
// as the index for a "switch", whereas Miden so far only has a 32-bit br_table.
...
// miden64. Here we fix that.
void fixBrTableIndex(MachineInstr &MI, MachineBasicBlock *MBB,
                     MachineFunction &MF) {
  // Only happens on miden64.
  auto &WST = MF.getSubtarget<MidenSubtarget>();
  if (!WST.hasAddr64())
    return;

  assert(MI.getDesc().getOpcode() == Miden::BR_TABLE_I64 &&
  ...
  if (ExtMI->getOpcode() == Miden::I64_EXTEND_U_I32) {
    ...
    BuildMI(*MBB, MI.getIterator(), MI.getDebugLoc(),
            WST.getInstrInfo()->get(Miden::I32_WRAP_I64), Reg32)
  ...
  // accordingly.
  MI.setDesc(WST.getInstrInfo()->get(Miden::BR_TABLE_I32));
}
...
MachineBasicBlock *fixBrTableDefault(MachineInstr &MI, MachineBasicBlock *MBB,
  ...
  if (TBB && TBB != MBB) {
    ...
    if (RangeCheck->getOpcode() != Miden::GT_U_I32)
      return nullptr;
...
bool MidenFixBrTableDefaults::runOnMachineFunction(MachineFunction &MF) {
...
INITIALIZE_PASS(MidenFixBrTableDefaults, DEBUG_TYPE,
                "Removes range checks and sets br_table default targets", false,
                false)

FunctionPass *llvm::createMidenFixBrTableDefaults() {
  return new MidenFixBrTableDefaults();
}
