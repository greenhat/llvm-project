//==- MidenAsmParser.cpp - Assembler for Miden -*- C++ -*-==//
//
...
/// MidenOperand - Instances of this class represent the operands in a
/// parsed Miden machine instruction.
struct MidenOperand : public MCParsedAsmOperand {
  ...
  MidenOperand(KindTy K, SMLoc Start, SMLoc End, TokOp T)
      : Kind(K), StartLoc(Start), EndLoc(End), Tok(T) {}
  MidenOperand(KindTy K, SMLoc Start, SMLoc End, IntOp I)
      : Kind(K), StartLoc(Start), EndLoc(End), Int(I) {}
  MidenOperand(KindTy K, SMLoc Start, SMLoc End, FltOp F)
      : Kind(K), StartLoc(Start), EndLoc(End), Flt(F) {}
  MidenOperand(KindTy K, SMLoc Start, SMLoc End, SymOp S)
      : Kind(K), StartLoc(Start), EndLoc(End), Sym(S) {}
  MidenOperand(KindTy K, SMLoc Start, SMLoc End)
...
static miden::MidenLimits DefaultLimits() {
  return {miden::MIDEN_LIMITS_FLAG_NONE, 0, 0};
}

static MCSymbolMiden *GetOrCreateFunctionTableSymbol(MCContext &Ctx,
...
class MidenAsmParser final : public MCTargetAsmParser {
  MCAsmParser &Parser;
  MCAsmLexer &Lexer;

  // Much like MidenAsmPrinter in the backend, we have to own these.
  ...
  MCSymbolMiden *DefaultFunctionTable = nullptr;
  MCSymbol *LastFunctionLabel = nullptr;

  bool is64;

  MidenAsmTypeCheck TC;
  // Don't type check if -no-type-check was set.
  bool SkipTypeCheck;

public:
  MidenAsmParser(const MCSubtargetInfo &STI, MCAsmParser &Parser,
...
#include "MidenGenAsmMatcher.inc"
  ...
  void push(NestingType NT) {
    NestingStack.push_back({NT, miden::MidenSignature()});
  }
  ...
  bool parseRegTypeList(SmallVectorImpl<miden::ValType> &Types) {
    while (Lexer.is(AsmToken::Identifier)) {
      auto Type = Miden::parseType(Lexer.getTok().getString());
      if (!Type)
  ...
  void parseSingleInteger(bool IsNegative, OperandVector &Operands) {
    ...
    if (IsNegative)
      Val = -Val;
    Operands.push_back(std::make_unique<MidenOperand>(
        MidenOperand::Integer, Int.getLoc(), Int.getEndLoc(),
        MidenOperand::IntOp{Val}));
    Parser.Lex();
  }
  ...
  bool parseSingleFloat(bool IsNegative, OperandVector &Operands) {
    ...
    if (IsNegative)
      Val = -Val;
    Operands.push_back(std::make_unique<MidenOperand>(
        MidenOperand::Float, Flt.getLoc(), Flt.getEndLoc(),
        MidenOperand::FltOp{Val}));
    Parser.Lex();
    return false;
  }
  ...
  bool checkForP2AlignIfLoadStore(OperandVector &Operands, StringRef InstName) {
    ...
    if (IsLoadStore || IsAtomic) {
      ...
      if (IsLoadStore && isNext(AsmToken::Colon)) {
        ...
        // We can't just call Miden::GetDefaultP2Align since we don't have
        // an opcode until after the assembly matcher, so set a default to fix
        // up later.
        auto Tok = Lexer.getTok();
        Operands.push_back(std::make_unique<MidenOperand>(
            MidenOperand::Integer, Tok.getLoc(), Tok.getEndLoc(),
            MidenOperand::IntOp{-1}));
      }
    }
    return false;
  }
  ...
  void addBlockTypeOperand(OperandVector &Operands, SMLoc NameLoc,
                           Miden::BlockType BT) {
    if (BT != Miden::BlockType::Void) {
      miden::MidenSignature Sig({static_cast<miden::ValType>(BT)}, {});
      TC.setLastSig(Sig);
      NestingStack.back().Sig = Sig;
    }
    Operands.push_back(std::make_unique<MidenOperand>(
        MidenOperand::Integer, NameLoc, NameLoc,
        MidenOperand::IntOp{static_cast<int64_t>(BT)}));
  }

  bool parseLimits(miden::MidenLimits *Limits) {
    auto Tok = Lexer.getTok();
    ...
    if (isNext(AsmToken::Comma)) {
      Limits->Flags |= miden::MIDEN_LIMITS_FLAG_HAS_MAX;
      auto Tok = Lexer.getTok();
      if (!Tok.is(AsmToken::Integer))
        return error("Expected integer constant, instead got: ", Tok);
      int64_t Val = Tok.getIntVal();
  ...
  bool parseFunctionTableOperand(std::unique_ptr<MidenOperand> *Op) {
    if (STI->checkFeatures("+reference-types")) {
      ...
      if (Tok.is(AsmToken::Identifier)) {
        ...
        *Op = std::make_unique<MidenOperand>(MidenOperand::Symbol, Tok.getLoc(),
                                             Tok.getEndLoc(),
                                             MidenOperand::SymOp{Val});
        Parser.Lex();
        return expect(AsmToken::Comma, ",");
      } else {
        const auto *Val =
            MCSymbolRefExpr::create(DefaultFunctionTable, getContext());
        *Op = std::make_unique<MidenOperand>(MidenOperand::Symbol, SMLoc(),
                                             SMLoc(), MidenOperand::SymOp{Val});
        return false;
      }
    } else {
      ...
      *Op = std::make_unique<MidenOperand>(MidenOperand::Integer, SMLoc(),
                                           SMLoc(), MidenOperand::IntOp{0});
      return false;
    }
  }
  ...
  bool ParseInstruction(ParseInstructionInfo & /*Info*/, StringRef Name,
    ...
    // Miden has instructions with / in them, which AsmLexer parses
    ...
    Operands.push_back(std::make_unique<MidenOperand>(
        MidenOperand::Token, NameLoc, SMLoc::getFromPointer(Name.end()),
        MidenOperand::TokOp{Name}));

    // If this instruction is part of a control flow structure, ensure
    // proper nesting.
    bool ExpectBlockType = false;
    bool ExpectFuncType = false;
    std::unique_ptr<MidenOperand> FunctionTable;
    if (Name == "block") {
      push(Block);
    ...
    if (ExpectFuncType || (ExpectBlockType && Lexer.is(AsmToken::LParen))) {
      ...
      // attach it to an anonymous symbol, which is what MidenObjectWriter
      // expects to be able to recreate the actual unique-ified type indices.
      auto Loc = Parser.getTok();
      auto Signature = std::make_unique<miden::MidenSignature>();
      ...
      auto *MidenSym = cast<MCSymbolMiden>(Sym);
      MidenSym->setSignature(Signature.get());
      addSignature(std::move(Signature));
      MidenSym->setType(miden::MIDEN_SYMBOL_TYPE_FUNCTION);
      const MCExpr *Expr = MCSymbolRefExpr::create(
          MidenSym, MCSymbolRefExpr::VK_MIDEN_TYPEINDEX, Ctx);
      Operands.push_back(std::make_unique<MidenOperand>(
          MidenOperand::Symbol, Loc.getLoc(), Loc.getEndLoc(),
          MidenOperand::SymOp{Expr}));
    }
    ...
    while (Lexer.isNot(AsmToken::EndOfStatement)) {
      ...
      case AsmToken::Identifier: {
        ...
        if (ExpectBlockType) {
          ...
          auto BT = Miden::parseBlockType(Id.getString());
          if (BT == Miden::BlockType::Invalid)
          ...
          Operands.push_back(std::make_unique<MidenOperand>(
              MidenOperand::Symbol, Start, End, MidenOperand::SymOp{Val}));
      ...
      case AsmToken::LCurly: {
        Parser.Lex();
        auto Op = std::make_unique<MidenOperand>(MidenOperand::BrList,
    ...
    if (ExpectBlockType && Operands.size() == 1) {
      ...
      addBlockTypeOperand(Operands, NameLoc, Miden::BlockType::Void);
    }
    if (FunctionTable)
  ...
  bool parseSignature(miden::MidenSignature *Signature) {
  ...
  bool CheckDataSection() {
    if (CurrentState != DataSection) {
      auto WS = cast<MCSectionMiden>(getStreamer().getCurrentSection().first);
  ...
  // This function processes miden-specific directives streamed to
  // MidenTargetStreamer, all others go to the generic parser
  // (see MidenAsmParser).
  bool ParseDirective(AsmToken DirectiveID) override {
    ...
    auto &TOut =
        reinterpret_cast<MidenTargetStreamer &>(*Out.getTargetStreamer());
    ...
    if (DirectiveID.getString() == ".globaltype") {
      ...
      if (TypeName.empty())
        return true;
      auto Type = Miden::parseType(TypeName);
      if (!Type)
      ...
      auto MidenSym = cast<MCSymbolMiden>(Ctx.getOrCreateSymbol(SymName));
      MidenSym->setType(miden::MIDEN_SYMBOL_TYPE_GLOBAL);
      MidenSym->setGlobalType(miden::MidenGlobalType{uint8_t(*Type), Mutable});
      // And emit the directive again.
      TOut.emitGlobalType(MidenSym);
    ...
    if (DirectiveID.getString() == ".tabletype") {
      ...
      if (ElemTypeName.empty())
        return true;
      Optional<miden::ValType> ElemType = Miden::parseType(ElemTypeName);
      if (!ElemType)
        return error("Unknown type in .tabletype directive: ", ElemTypeTok);

      miden::MidenLimits Limits = DefaultLimits();
      ...
      // symbol
      auto MidenSym = cast<MCSymbolMiden>(Ctx.getOrCreateSymbol(SymName));
      MidenSym->setType(miden::MIDEN_SYMBOL_TYPE_TABLE);
      miden::MidenTableType Type = {uint8_t(*ElemType), Limits};
      MidenSym->setTableType(Type);
      TOut.emitTableType(MidenSym);
      return expect(AsmToken::EndOfStatement, "EOL");
    }

    if (DirectiveID.getString() == ".functype") {
      // This code has to send things to the streamer similar to
      // MidenAsmPrinter::EmitFunctionBodyStart.
      ...
      if (SymName.empty())
        return true;
      auto MidenSym = cast<MCSymbolMiden>(Ctx.getOrCreateSymbol(SymName));
      if (MidenSym->isDefined()) {
        // This .functype indicates a start of a function.
        if (ensureEmptyNestingStack())
          return true;
        CurrentState = FunctionStart;
        LastFunctionLabel = MidenSym;
        push(Function);
      }
      auto Signature = std::make_unique<miden::MidenSignature>();
      if (parseSignature(Signature.get()))
      ...
      addSignature(std::move(Signature));
      MidenSym->setType(miden::MIDEN_SYMBOL_TYPE_FUNCTION);
      TOut.emitFunctionType(MidenSym);
    ...
    if (DirectiveID.getString() == ".export_name") {
      ...
      auto MidenSym = cast<MCSymbolMiden>(Ctx.getOrCreateSymbol(SymName));
      MidenSym->setExportName(storeName(ExportName));
      TOut.emitExportName(MidenSym, ExportName);
    }
    ...
    if (DirectiveID.getString() == ".import_module") {
      ...
      auto MidenSym = cast<MCSymbolMiden>(Ctx.getOrCreateSymbol(SymName));
      MidenSym->setImportModule(storeName(ImportModule));
      TOut.emitImportModule(MidenSym, ImportModule);
    }
    ...
    if (DirectiveID.getString() == ".import_name") {
      ...
      auto MidenSym = cast<MCSymbolMiden>(Ctx.getOrCreateSymbol(SymName));
      MidenSym->setImportName(storeName(ImportName));
      TOut.emitImportName(MidenSym, ImportName);
    }
    ...
    if (DirectiveID.getString() == ".local") {
      if (CurrentState != FunctionStart)
        return error(".local directive should follow the start of a function: ",
                     Lexer.getTok());
      SmallVector<miden::ValType, 4> Locals;
  ...
  void ensureLocals(MCStreamer &Out) {
    if (CurrentState == FunctionStart) {
      ...
      auto &TOut =
          reinterpret_cast<MidenTargetStreamer &>(*Out.getTargetStreamer());
      TOut.emitLocal(SmallVector<miden::ValType, 0>());
      CurrentState = FunctionLocals;
    }
  }
  ...
  bool MatchAndEmitInstruction(SMLoc IDLoc, unsigned & /*Opcode*/,
    ...
    case Match_Success: {
      ...
      auto Align = Miden::GetDefaultP2AlignAny(Inst.getOpcode());
      if (Align != -1U) {
      ...
      if (is64) {
        ...
        auto Opc64 =
            Miden::getMiden64Opcode(static_cast<uint16_t>(Inst.getOpcode()));
        if (Opc64 >= 0) {
  ...
  void doBeforeLabelEmit(MCSymbol *Symbol) override {
    ...
    auto CWS = cast<MCSectionMiden>(getStreamer().getCurrentSection().first);
    if (!CWS || !CWS->getKind().isText())
      return;

    auto MidenSym = cast<MCSymbolMiden>(Symbol);
    // Unlike other targets, we don't allow data in text sections (labels
    // declared with .type @object).
    if (MidenSym->getType() == miden::MIDEN_SYMBOL_TYPE_DATA) {
      Parser.Error(Parser.getTok().getLoc(),
                   "Miden doesn\'t support data symbols in text sections");
      return;
    }
    ...
    // (that requires coordination with MidenAsmParser::parseSectionDirective)
    ...
    // assembly currently.
    if (Group)
      MidenSym->setComdat(true);
    auto *WS =
        getContext().getMidenSection(SecName, SectionKind::getText(), 0, Group,
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeMidenAsmParser() {
  RegisterMCAsmParser<MidenAsmParser> X(getTheMidenTarget32());
  RegisterMCAsmParser<MidenAsmParser> Y(getTheMidenTarget64());
}
