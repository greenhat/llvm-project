//== MidenMemIntrinsicResults.cpp - Optimize memory intrinsic results ==//
//
...
#define DEBUG_TYPE "miden-mem-intrinsic-results"

namespace {
class MidenMemIntrinsicResults final : public MachineFunctionPass {
public:
  static char ID; // Pass identification, replacement for typeid
  MidenMemIntrinsicResults() : MachineFunctionPass(ID) {}

  StringRef getPassName() const override {
    return "Miden Memory Intrinsic Results";
  }
...
char MidenMemIntrinsicResults::ID = 0;
INITIALIZE_PASS(MidenMemIntrinsicResults, DEBUG_TYPE,
                "Optimize memory intrinsic result values for Miden", false,
                false)

FunctionPass *llvm::createMidenMemIntrinsicResults() {
  return new MidenMemIntrinsicResults();
}
...
static bool replaceDominatedUses(MachineBasicBlock &MBB, MachineInstr &MI,
  ...
  if (Changed) {
    ...
    if (!FromLI->liveAt(FromIdx.getDeadSlot()))
      MI.addRegisterKilled(
          FromReg,
          MBB.getParent()->getSubtarget<MidenSubtarget>().getRegisterInfo());
  }
...
bool MidenMemIntrinsicResults::runOnMachineFunction(MachineFunction &MF) {
  LLVM_DEBUG({
