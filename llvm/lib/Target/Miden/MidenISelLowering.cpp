//=- MidenISelLowering.cpp - Miden DAG Lowering Implementation -==//
//
...
///
/// \file
/// This file implements the MidenTargetLowering class.
///
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "miden-lower"

MidenTargetLowering::MidenTargetLowering(const TargetMachine &TM,
                                         const MidenSubtarget &STI)
  ...
  setStackPointerRegisterToSaveRestore(Subtarget->hasAddr64() ? Miden::SP64
                                                              : Miden::SP32);
  // Set up the register classes.
  addRegisterClass(MVT::i32, &Miden::I32RegClass);
  addRegisterClass(MVT::i64, &Miden::I64RegClass);
  addRegisterClass(MVT::f32, &Miden::F32RegClass);
  addRegisterClass(MVT::f64, &Miden::F64RegClass);
  if (Subtarget->hasSIMD128()) {
    addRegisterClass(MVT::v16i8, &Miden::V128RegClass);
    addRegisterClass(MVT::v8i16, &Miden::V128RegClass);
    addRegisterClass(MVT::v4i32, &Miden::V128RegClass);
    addRegisterClass(MVT::v4f32, &Miden::V128RegClass);
    addRegisterClass(MVT::v2i64, &Miden::V128RegClass);
    addRegisterClass(MVT::v2f64, &Miden::V128RegClass);
  }
  if (Subtarget->hasReferenceTypes()) {
    addRegisterClass(MVT::externref, &Miden::EXTERNREFRegClass);
    addRegisterClass(MVT::funcref, &Miden::FUNCREFRegClass);
  }
  // Compute derived properties from the register classes.
  computeRegisterProperties(Subtarget->getRegisterInfo());

  // Transform loads and stores to pointers in address space 1 to loads and
  // stores to Miden global variables, outside linear memory.
  ...
  for (auto T : {MVT::f32, MVT::f64, MVT::v4f32, MVT::v2f64}) {
    ...
    // Miden currently has no builtin f16 support.
  ...
  if (Subtarget->hasSIMD128()) {
    ...
    // Custom lowering since miden shifts must have a scalar shift amount
  ...
  // Trap lowers to miden unreachable
  ...
  // TODO: when implementing other Miden backends, make this generic or only do
  ...
  // is equivalent to a simple branch. This reduces code size for miden, and we
  // defer possible jump table optimizations to the VM.
  setMinimumJumpTableEntries(2);
}

MVT MidenTargetLowering::getPointerTy(const DataLayout &DL, uint32_t AS) const {
  if (AS == Miden::MidenAddressSpace::MIDEN_ADDRESS_SPACE_EXTERNREF)
    return MVT::externref;
  if (AS == Miden::MidenAddressSpace::MIDEN_ADDRESS_SPACE_FUNCREF)
    return MVT::funcref;
  return TargetLowering::getPointerTy(DL, AS);
}

MVT MidenTargetLowering::getPointerMemTy(const DataLayout &DL,
                                         uint32_t AS) const {
  if (AS == Miden::MidenAddressSpace::MIDEN_ADDRESS_SPACE_EXTERNREF)
    return MVT::externref;
  if (AS == Miden::MidenAddressSpace::MIDEN_ADDRESS_SPACE_FUNCREF)
...
MidenTargetLowering::shouldExpandAtomicRMWInIR(AtomicRMWInst *AI) const {
  // We have miden instructions for these
...
bool MidenTargetLowering::shouldScalarizeBinop(SDValue VecOp) const {
...
FastISel *
MidenTargetLowering::createFastISel(FunctionLoweringInfo &FuncInfo,
                                    const TargetLibraryInfo *LibInfo) const {
  return Miden::createFastISel(FuncInfo, LibInfo);
}

MVT MidenTargetLowering::getScalarShiftAmountTy(const DataLayout & /*DL*/,
                                                EVT VT) const {
...
// undefined result on invalid/overflow, to the Miden opcode, which
...
static MachineBasicBlock *LowerFPToInt(MachineInstr &MI, DebugLoc DL,
  ...
  unsigned Abs = Float64 ? Miden::ABS_F64 : Miden::ABS_F32;
  unsigned FConst = Float64 ? Miden::CONST_F64 : Miden::CONST_F32;
  unsigned LT = Float64 ? Miden::LT_F64 : Miden::LT_F32;
  unsigned GE = Float64 ? Miden::GE_F64 : Miden::GE_F32;
  unsigned IConst = Int64 ? Miden::CONST_I64 : Miden::CONST_I32;
  unsigned Eqz = Miden::EQZ_I32;
  unsigned And = Miden::AND_I32;
  ...
  CmpReg = MRI.createVirtualRegister(&Miden::I32RegClass);
  EqzReg = MRI.createVirtualRegister(&Miden::I32RegClass);
  ...
  if (IsUnsigned) {
    Tmp1 = MRI.createVirtualRegister(MRI.getRegClass(InReg));
    Register SecondCmpReg = MRI.createVirtualRegister(&Miden::I32RegClass);
    Register AndReg = MRI.createVirtualRegister(&Miden::I32RegClass);
    BuildMI(BB, DL, TII.get(FConst), Tmp1)
  ...
  BuildMI(BB, DL, TII.get(Miden::BR_IF)).addMBB(TrueMBB).addReg(EqzReg);
  BuildMI(FalseMBB, DL, TII.get(LoweredOpcode), FalseReg).addReg(InReg);
  BuildMI(FalseMBB, DL, TII.get(Miden::BR)).addMBB(DoneMBB);
...
static MachineBasicBlock *LowerCallResults(MachineInstr &CallResults,
  ...
  assert(CallParams.getOpcode() == Miden::CALL_PARAMS);
  assert(CallResults.getOpcode() == Miden::CALL_RESULTS ||
         CallResults.getOpcode() == Miden::RET_CALL_RESULTS);

  bool IsIndirect = CallParams.getOperand(0).isReg();
  bool IsRetCall = CallResults.getOpcode() == Miden::RET_CALL_RESULTS;
  ...
  if (IsIndirect) {
    ...
    IsFuncrefCall = (TRC == &Miden::FUNCREFRegClass);
    assert(!IsFuncrefCall || Subtarget->hasReferenceTypes());
  }

  unsigned CallOp;
  if (IsIndirect && IsRetCall) {
    CallOp = Miden::RET_CALL_INDIRECT;
  } else if (IsIndirect) {
    CallOp = Miden::CALL_INDIRECT;
  } else if (IsRetCall) {
    CallOp = Miden::RET_CALL;
  } else {
    CallOp = Miden::CALL;
  }
  ...
  // CALL_INDIRECT takes an i32, but in miden64 we represent function pointers
  // as 64-bit for uniformity with other pointer types.
  // See also: MidenFastISel::selectCall
  if (IsIndirect && MF.getSubtarget<MidenSubtarget>().hasAddr64()) {
    Register Reg32 = MF.getRegInfo().createVirtualRegister(&Miden::I32RegClass);
    auto &FnPtr = CallParams.getOperand(0);
    BuildMI(*BB, CallResults.getIterator(), DL, TII.get(Miden::I32_WRAP_I64),
            Reg32)
        .addReg(FnPtr.getReg());
    FnPtr.setReg(Reg32);
  }
  ...
  if (IsIndirect) {
    ...
    if (IsFuncrefCall) {
      Register RegZero =
          MF.getRegInfo().createVirtualRegister(&Miden::I32RegClass);
      MachineInstrBuilder MIBC0 =
          BuildMI(MF, DL, TII.get(Miden::CONST_I32), RegZero).addImm(0);
  ...
  if (IsIndirect) {
    ...
    MCSymbolMiden *Table =
        IsFuncrefCall
            ? Miden::getOrCreateFuncrefCallTableSymbol(MF.getContext(),
                                                       Subtarget)
            : Miden::getOrCreateFunctionTableSymbol(MF.getContext(), Subtarget);
    if (Subtarget->hasReferenceTypes()) {
      MIB.addSym(Table);
    } else {
  ...
  if (IsIndirect && IsFuncrefCall) {
    MCSymbolMiden *Table =
        Miden::getOrCreateFuncrefCallTableSymbol(MF.getContext(), Subtarget);
    Register RegZero =
        MF.getRegInfo().createVirtualRegister(&Miden::I32RegClass);
    MachineInstr *Const0 =
        BuildMI(MF, DL, TII.get(Miden::CONST_I32), RegZero).addImm(0);
    ...
    Register RegFuncref =
        MF.getRegInfo().createVirtualRegister(&Miden::FUNCREFRegClass);
    MachineInstr *RefNull =
        BuildMI(MF, DL, TII.get(Miden::REF_NULL_FUNCREF), RegFuncref);
    BB->insertAfter(Const0->getIterator(), RefNull);

    MachineInstr *TableSet = BuildMI(MF, DL, TII.get(Miden::TABLE_SET_FUNCREF))
                                 .addSym(Table)
                                 .addReg(RegZero)
                                 .addReg(RegFuncref);
...
MachineBasicBlock *
MidenTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
  ...
  case Miden::FP_TO_SINT_I32_F32:
    return LowerFPToInt(MI, DL, BB, TII, false, false, false,
                        Miden::I32_TRUNC_S_F32);
  case Miden::FP_TO_UINT_I32_F32:
    return LowerFPToInt(MI, DL, BB, TII, true, false, false,
                        Miden::I32_TRUNC_U_F32);
  case Miden::FP_TO_SINT_I64_F32:
    return LowerFPToInt(MI, DL, BB, TII, false, true, false,
                        Miden::I64_TRUNC_S_F32);
  case Miden::FP_TO_UINT_I64_F32:
    return LowerFPToInt(MI, DL, BB, TII, true, true, false,
                        Miden::I64_TRUNC_U_F32);
  case Miden::FP_TO_SINT_I32_F64:
    return LowerFPToInt(MI, DL, BB, TII, false, false, true,
                        Miden::I32_TRUNC_S_F64);
  case Miden::FP_TO_UINT_I32_F64:
    return LowerFPToInt(MI, DL, BB, TII, true, false, true,
                        Miden::I32_TRUNC_U_F64);
  case Miden::FP_TO_SINT_I64_F64:
    return LowerFPToInt(MI, DL, BB, TII, false, true, true,
                        Miden::I64_TRUNC_S_F64);
  case Miden::FP_TO_UINT_I64_F64:
    return LowerFPToInt(MI, DL, BB, TII, true, true, true,
                        Miden::I64_TRUNC_U_F64);
  case Miden::CALL_RESULTS:
  case Miden::RET_CALL_RESULTS:
    return LowerCallResults(MI, DL, BB, Subtarget, TII);
  }
}

const char *MidenTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (static_cast<MidenISD::NodeType>(Opcode)) {
  case MidenISD::FIRST_NUMBER:
  case MidenISD::FIRST_MEM_OPCODE:
    break;
#define HANDLE_NODETYPE(NODE)                                                  \
  case MidenISD::NODE:                                                         \
    return "MidenISD::" #NODE;
#define HANDLE_MEM_NODETYPE(NODE) HANDLE_NODETYPE(NODE)
#include "MidenISD.def"
...
MidenTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
                                                  StringRef Constraint,
                                                  MVT VT) const {
  // First, see if this is a constraint that directly corresponds to a
  // Miden register class.
    ...
    case 'r':
      ...
      if (Subtarget->hasSIMD128() && VT.isVector()) {
        if (VT.getSizeInBits() == 128)
          return std::make_pair(0U, &Miden::V128RegClass);
      }
      if (VT.isInteger() && !VT.isVector()) {
        if (VT.getSizeInBits() <= 32)
          return std::make_pair(0U, &Miden::I32RegClass);
        if (VT.getSizeInBits() <= 64)
          return std::make_pair(0U, &Miden::I64RegClass);
      }
      if (VT.isFloatingPoint() && !VT.isVector()) {
        switch (VT.getSizeInBits()) {
        case 32:
          return std::make_pair(0U, &Miden::F32RegClass);
        case 64:
          return std::make_pair(0U, &Miden::F64RegClass);
        default:
          break;
        }
      }
...
bool MidenTargetLowering::isCheapToSpeculateCttz() const {
  // Assume ctz is a relatively cheap operation.
  return true;
}

bool MidenTargetLowering::isCheapToSpeculateCtlz() const {
  // Assume clz is a relatively cheap operation.
  return true;
}

bool MidenTargetLowering::isLegalAddressingMode(const DataLayout &DL,
                                                const AddrMode &AM, Type *Ty,
                                                unsigned AS,
                                                Instruction *I) const {
  // Miden offsets are added as unsigned without wrapping. The
  // isLegalAddressingMode gives us no way to determine if wrapping could be
  // happening, so we approximate this by accepting only non-negative offsets.
  if (AM.BaseOffs < 0)
    return false;

  // Miden has no scale register operands.
  if (AM.Scale != 0)
    return false;
...
bool MidenTargetLowering::allowsMisalignedMemoryAccesses(
    EVT /*VT*/, unsigned /*AddrSpace*/, Align /*Align*/,
    MachineMemOperand::Flags /*Flags*/, bool *Fast) const {
  // Miden supports unaligned accesses, though it should be declared
  ...
  // of constants, etc.), Miden implementations will either want the
...
bool MidenTargetLowering::isIntDivCheap(EVT VT, AttributeList Attr) const {
  // The current thinking is that miden engines will perform this optimization,
  // so we can save on code size.
  return true;
}

bool MidenTargetLowering::isVectorLoadExtDesirable(SDValue ExtVal) const {
...
bool MidenTargetLowering::isOffsetFoldingLegal(
    const GlobalAddressSDNode *GA) const {
  // Miden doesn't support function addresses with offsets
  const GlobalValue *GV = GA->getGlobal();
  return isa<Function>(GV) ? false : TargetLowering::isOffsetFoldingLegal(GA);
}

EVT MidenTargetLowering::getSetCCResultType(const DataLayout &DL,
                                            LLVMContext &C, EVT VT) const {
  if (VT.isVector())
    return VT.changeVectorElementTypeToInteger();

  // So far, all branch instructions in Miden take an I32 condition.
  ...
  // against 64-bit pointers/values if at some point Miden supports that.
  return EVT::getIntegerVT(C, 32);
}

bool MidenTargetLowering::getTgtMemIntrinsic(IntrinsicInfo &Info,
                                             const CallInst &I,
                                             MachineFunction &MF,
                                             unsigned Intrinsic) const {
  switch (Intrinsic) {
  case Intrinsic::miden_memory_atomic_notify:
    ...
    // consistent. The same applies for miden_atomic_wait intrinsics too.
    Info.flags = MachineMemOperand::MOVolatile | MachineMemOperand::MOLoad;
    return true;
  case Intrinsic::miden_memory_atomic_wait32:
    Info.opc = ISD::INTRINSIC_W_CHAIN;
    Info.memVT = MVT::i32;
    Info.ptrVal = I.getArgOperand(0);
    Info.offset = 0;
    ...
    Info.flags = MachineMemOperand::MOVolatile | MachineMemOperand::MOLoad;
    return true;
  case Intrinsic::miden_memory_atomic_wait64:
    Info.opc = ISD::INTRINSIC_W_CHAIN;
    Info.memVT = MVT::i64;
    Info.ptrVal = I.getArgOperand(0);
    Info.offset = 0;
...
void MidenTargetLowering::computeKnownBitsForTargetNode(
...
MidenTargetLowering::getPreferredVectorAction(MVT VT) const {
...
bool MidenTargetLowering::shouldSimplifyDemandedVectorElts(
...
//===----------------------------------------------------------------------===//
// Miden Lowering private implementation.
//===----------------------------------------------------------------------===//
...
static bool callingConvSupported(CallingConv::ID CallConv) {
  ...
  return CallConv == CallingConv::C || CallConv == CallingConv::Fast ||
         ...
         // CallConv == CallingConv::MIDEN_EmscriptenInvoke ||
         CallConv == CallingConv::Swift;
}

SDValue MidenTargetLowering::LowerCall(CallLoweringInfo &CLI,
  ...
  if (!callingConvSupported(CallConv))
    fail(DL, DAG,
         "Miden doesn't support language-specific or target-specific "
         "calling conventions yet");
  if (CLI.IsPatchPoint)
    fail(DL, DAG, "Miden doesn't support patch point yet");
  ...
  if (CLI.IsTailCall) {
    ...
    if (!Subtarget->hasTailCall())
      NoTail("Miden 'tail-call' feature not enabled");

    // Varargs calls cannot be tail calls because the buffer is on the stack
    if (CLI.IsVarArg)
      NoTail("Miden does not support varargs tail calls");
    ...
    bool TypesMatch = CallerRetTys.size() == CalleeRetTys.size() &&
                      std::equal(CallerRetTys.begin(), CallerRetTys.end(),
                                 CalleeRetTys.begin());
    if (!TypesMatch)
      NoTail("Miden tail call requires caller and callee return types to "
             "match");
    ...
    if (CLI.CB) {
      for (auto &Arg : CLI.CB->args()) {
        ...
        if (isa<AllocaInst>(Val)) {
          NoTail("Miden does not support tail calling with stack arguments");
          break;
        }
      }
    }
  ...
  // if (CallConv == CallingConv::MIDEN_EmscriptenInvoke && Outs.size() >= 2 &&
  //     Outs[0].Flags.isSRet()) {
  ...
  for (unsigned I = 0; I < Outs.size(); ++I) {
    ...
    if (Out.Flags.isNest())
      fail(DL, DAG, "Miden hasn't implemented nest arguments");
    if (Out.Flags.isInAlloca())
      fail(DL, DAG, "Miden hasn't implemented inalloca arguments");
    if (Out.Flags.isInConsecutiveRegs())
      fail(DL, DAG, "Miden hasn't implemented cons regs arguments");
    if (Out.Flags.isInConsecutiveRegsLast())
      fail(DL, DAG, "Miden hasn't implemented cons regs last arguments");
  ...
  if (Callee->getOpcode() == ISD::GlobalAddress) {
    ...
    Callee = DAG.getTargetGlobalAddress(GA->getGlobal(), DL,
                                        ...
                                        GA->getOffset());
    Callee = DAG.getNode(MidenISD::Wrapper, DL,
  ...
  for (const auto &In : Ins) {
    ...
    if (In.Flags.isInAlloca())
      fail(DL, DAG, "Miden hasn't implemented inalloca return values");
    if (In.Flags.isInConsecutiveRegs())
      fail(DL, DAG, "Miden hasn't implemented cons regs return values");
    if (In.Flags.isInConsecutiveRegsLast())
      fail(DL, DAG, "Miden hasn't implemented cons regs last return values");
  ...
  if (CLI.CB && Miden::isFuncrefType(CLI.CB->getCalledOperand()->getType())) {
    ...
    SDValue TableSet = DAG.getMemIntrinsicNode(
        MidenISD::TABLE_SET, DL, DAG.getVTList(MVT::Other), TableSetOps,
        MVT::funcref,
        // Machine Mem Operand args
        MachinePointerInfo(
            Miden::MidenAddressSpace::MIDEN_ADDRESS_SPACE_FUNCREF),
  ...
  if (CLI.IsTailCall) {
    ...
    return DAG.getNode(MidenISD::RET_CALL, DL, NodeTys, Ops);
  }

  InTys.push_back(MVT::Other);
  SDVTList InTyList = DAG.getVTList(InTys);
  SDValue Res = DAG.getNode(MidenISD::CALL, DL, InTyList, Ops);
...
bool MidenTargetLowering::CanLowerReturn(
    CallingConv::ID /*CallConv*/, MachineFunction & /*MF*/, bool /*IsVarArg*/,
    const SmallVectorImpl<ISD::OutputArg> &Outs,
    LLVMContext & /*Context*/) const {
  // Miden can only handle returning tuples with multivalue enabled
  return Subtarget->hasMultivalue() || Outs.size() <= 1;
}

SDValue
MidenTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                 bool /*IsVarArg*/,
                                 const SmallVectorImpl<ISD::OutputArg> &Outs,
                                 const SmallVectorImpl<SDValue> &OutVals,
                                 const SDLoc &DL, SelectionDAG &DAG) const {
  assert((Subtarget->hasMultivalue() || Outs.size() <= 1) &&
         "MVP Miden can only return up to one value");
  if (!callingConvSupported(CallConv))
    fail(DL, DAG, "Miden doesn't support non-C calling conventions");

  SmallVector<SDValue, 4> RetOps(1, Chain);
  RetOps.append(OutVals.begin(), OutVals.end());
  Chain = DAG.getNode(MidenISD::RETURN, DL, MVT::Other, RetOps);
  ...
  for (const ISD::OutputArg &Out : Outs) {
    ...
    if (Out.Flags.isInAlloca())
      fail(DL, DAG, "Miden hasn't implemented inalloca results");
    if (Out.Flags.isInConsecutiveRegs())
      fail(DL, DAG, "Miden hasn't implemented cons regs results");
    if (Out.Flags.isInConsecutiveRegsLast())
      fail(DL, DAG, "Miden hasn't implemented cons regs last results");
  }

  return Chain;
}

SDValue MidenTargetLowering::LowerFormalArguments(
  ...
  MachineFunction &MF = DAG.getMachineFunction();
  auto *MFI = MF.getInfo<MidenFunctionInfo>();

  // Set up the incoming ARGUMENTS value, which serves to represent the liveness
  // of the incoming values before they're represented by virtual registers.
  MF.getRegInfo().addLiveIn(Miden::ARGUMENTS);
  ...
  for (const ISD::InputArg &In : Ins) {
    ...
    // Ignore In.getNonZeroOrigAlign() because all our arguments are passed in
    // registers.
    InVals.push_back(In.Used ? DAG.getNode(MidenISD::ARGUMENT, DL, In.VT,
  ...
  if (IsVarArg) {
    ...
    Chain = DAG.getCopyToReg(
        Chain, DL, VarargVreg,
        DAG.getNode(MidenISD::ARGUMENT, DL, PtrVT,
  ...
  for (MVT VT : Results)
    MFI->addResult(VT);
  // TODO: Use signatures in MidenMachineFunctionInfo too and unify
...
void MidenTargetLowering::ReplaceNodeResults(SDNode *N,
  ...
  default:
    llvm_unreachable(
        "ReplaceNodeResults not implemented for this op for Miden!");
  }
}
...
SDValue MidenTargetLowering::LowerOperation(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDLoc DL(Op);
  switch (Op.getOpcode()) {
  default:
  ...
  case ISD::BRIND:
    fail(DL, DAG, "Miden hasn't implemented computed gotos");
    return SDValue();
  case ISD::RETURNADDR:
...
static bool IsMidenGlobal(SDValue Op) {
  if (const GlobalAddressSDNode *GA = dyn_cast<GlobalAddressSDNode>(Op))
    return Miden::isMidenVarAddressSpace(GA->getAddressSpace());

  return false;
}

static Optional<unsigned> IsMidenLocal(SDValue Op, SelectionDAG &DAG) {
  const FrameIndexSDNode *FI = dyn_cast<FrameIndexSDNode>(Op);
  if (!FI)
    return None;

  auto &MF = DAG.getMachineFunction();
  return MidenFrameLowering::getLocalForStackObject(MF, FI->getIndex());
}

static bool IsMidenTable(SDValue Op) {
  const GlobalAddressSDNode *GA = dyn_cast<GlobalAddressSDNode>(Op);
  if (GA && Miden::isMidenVarAddressSpace(GA->getAddressSpace())) {
    const GlobalValue *Value = GA->getGlobal();
    const Type *Ty = Value->getValueType();

    if (Ty->isArrayTy() && Miden::isRefType(Ty->getArrayElementType()))
      return true;
  }
  return false;
}
...
static bool IsMidenTableWithOffset(SDValue Op) {
  if (Op->getOpcode() == ISD::ADD && Op->getNumOperands() == 2)
    return (Op->getOperand(1).getSimpleValueType() == MVT::i32 &&
            IsMidenTableWithOffset(Op->getOperand(0))) ||
           (Op->getOperand(0).getSimpleValueType() == MVT::i32 &&
            IsMidenTableWithOffset(Op->getOperand(1)));

  return IsMidenTable(Op);
}

// Helper for table pattern matching used in LowerStore and LowerLoad
bool MidenTargetLowering::MatchTableForLowering(SelectionDAG &DAG,
                                                const SDLoc &DL,
                                                const SDValue &Base,
...
SDValue MidenTargetLowering::LowerStore(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (IsMidenTableWithOffset(Base)) {
    if (!Offset->isUndef())
      report_fatal_error(
    ...
    SDValue TableSet =
        DAG.getMemIntrinsicNode(MidenISD::TABLE_SET, DL, Tys, TableSetOps,
                                SN->getMemoryVT(), SN->getMemOperand());
    return TableSet;
  }

  if (IsMidenGlobal(Base)) {
    if (!Offset->isUndef())
    ...
    return DAG.getMemIntrinsicNode(MidenISD::GLOBAL_SET, DL, Tys, Ops,
                                   SN->getMemoryVT(), SN->getMemOperand());
  }

  if (Optional<unsigned> Local = IsMidenLocal(Base, DAG)) {
    if (!Offset->isUndef())
    ...
    return DAG.getNode(MidenISD::LOCAL_SET, DL, Tys, Ops);
  }

  return Op;
}

SDValue MidenTargetLowering::LowerLoad(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (IsMidenTableWithOffset(Base)) {
    ...
    SDValue TableGet =
        DAG.getMemIntrinsicNode(MidenISD::TABLE_GET, DL, Tys, TableGetOps,
  ...
  if (IsMidenGlobal(Base)) {
    ...
    return DAG.getMemIntrinsicNode(MidenISD::GLOBAL_GET, DL, Tys, Ops,
                                   LN->getMemoryVT(), LN->getMemOperand());
  }
  ...
  if (Optional<unsigned> Local = IsMidenLocal(Base, DAG)) {
    ...
    SDValue LocalGet =
        DAG.getNode(MidenISD::LOCAL_GET, DL, LocalVT, {LN->getChain(), Idx});
...
SDValue MidenTargetLowering::LowerCopyToReg(SDValue Op,
                                            SelectionDAG &DAG) const {
  ...
  if (isa<FrameIndexSDNode>(Src.getNode())) {
    ...
    SDValue Copy(
        DAG.getMachineNode(VT == MVT::i32 ? Miden::COPY_I32 : Miden::COPY_I64,
                           DL, VT, Src),
        0);
...
SDValue MidenTargetLowering::LowerFrameIndex(SDValue Op,
                                             SelectionDAG &DAG) const {
  int FI = cast<FrameIndexSDNode>(Op)->getIndex();
  return DAG.getTargetFrameIndex(FI, Op.getValueType());
}

SDValue MidenTargetLowering::LowerRETURNADDR(SDValue Op,
                                             SelectionDAG &DAG) const {
  SDLoc DL(Op);

  if (!Subtarget->getTargetTriple().isOSEmscripten()) {
    fail(DL, DAG,
         "Non-Emscripten Miden hasn't implemented "
...
SDValue MidenTargetLowering::LowerFRAMEADDR(SDValue Op,
                                            SelectionDAG &DAG) const {
  // Non-zero depths are not supported by Miden currently. Use the
...
SDValue MidenTargetLowering::LowerGlobalTLSAddress(SDValue Op,
                                                   SelectionDAG &DAG) const {
  SDLoc DL(Op);
  const auto *GA = cast<GlobalAddressSDNode>(Op);

  MachineFunction &MF = DAG.getMachineFunction();
  if (!MF.getSubtarget<MidenSubtarget>().hasBulkMemory())
  ...
  if (model == GlobalValue::LocalExecTLSModel ||
    ...
    SDValue TLSOffset = DAG.getTargetGlobalAddress(
        GV, DL, PtrVT, GA->getOffset(), MidenII::MO_TLS_BASE_REL);
    SDValue SymOffset = DAG.getNode(MidenISD::WrapperREL, DL, PtrVT, TLSOffset);
  ...
  EVT VT = Op.getValueType();
  return DAG.getNode(MidenISD::Wrapper, DL, VT,
                     DAG.getTargetGlobalAddress(GA->getGlobal(), DL, VT,
                                                GA->getOffset(),
                                                MidenII::MO_GOT_TLS));
}

SDValue MidenTargetLowering::LowerGlobalAddress(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc DL(Op);
  const auto *GA = cast<GlobalAddressSDNode>(Op);
  EVT VT = Op.getValueType();
  ...
  if (!Miden::isValidAddressSpace(GA->getAddressSpace()))
    fail(DL, DAG, "Invalid address space for Miden target");
  ...
  if (isPositionIndependent()) {
    ...
    if (getTargetMachine().shouldAssumeDSOLocal(*GV->getParent(), GV)) {
      ...
      if (GV->getValueType()->isFunctionTy()) {
        ...
        OperandFlags = MidenII::MO_TABLE_BASE_REL;
      } else {
        BaseName = MF.createExternalSymbolName("__memory_base");
        OperandFlags = MidenII::MO_MEMORY_BASE_REL;
      }
      SDValue BaseAddr =
          DAG.getNode(MidenISD::Wrapper, DL, PtrVT,
                      DAG.getTargetExternalSymbol(BaseName, PtrVT));

      SDValue SymAddr = DAG.getNode(
          MidenISD::WrapperREL, DL, VT,
          DAG.getTargetGlobalAddress(GA->getGlobal(), DL, VT, GA->getOffset(),
                                     OperandFlags));

      return DAG.getNode(ISD::ADD, DL, VT, BaseAddr, SymAddr);
    }
    OperandFlags = MidenII::MO_GOT;
  }
...
SDValue MidenTargetLowering::LowerExternalSymbol(SDValue Op,
                                                 SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue MidenTargetLowering::LowerJumpTable(SDValue Op,
                                            SelectionDAG &DAG) const {
...
SDValue MidenTargetLowering::LowerBR_JT(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  SDValue Chain = Op.getOperand(0);
  const auto *JT = cast<JumpTableSDNode>(Op.getOperand(1));
  SDValue Index = Op.getOperand(2);
  assert(JT->getTargetFlags() == 0 && "Miden doesn't set target flags");
  ...
  // if possible by MidenFixBrTableDefaults.
  Ops.push_back(DAG.getBasicBlock(*MBBs.begin()));
  return DAG.getNode(MidenISD::BR_TABLE, DL, MVT::Other, Ops);
}

SDValue MidenTargetLowering::LowerVASTART(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  EVT PtrVT = getPointerTy(DAG.getMachineFunction().getDataLayout());

  auto *MFI = DAG.getMachineFunction().getInfo<MidenFunctionInfo>();
...
SDValue MidenTargetLowering::LowerIntrinsic(SDValue Op,
                                            SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  unsigned IntNo;
  switch (Op.getOpcode()) {
  ...
  case Intrinsic::miden_lsda: {
    ...
    if (isPositionIndependent()) {
      SDValue Node = DAG.getTargetExternalSymbol(SymName, PtrVT,
                                                 MidenII::MO_MEMORY_BASE_REL);
      const char *BaseName = MF.createExternalSymbolName("__memory_base");
      SDValue BaseAddr =
          DAG.getNode(MidenISD::Wrapper, DL, PtrVT,
                      DAG.getTargetExternalSymbol(BaseName, PtrVT));
      SDValue SymAddr = DAG.getNode(MidenISD::WrapperREL, DL, PtrVT, Node);
      return DAG.getNode(ISD::ADD, DL, PtrVT, BaseAddr, SymAddr);
    }
    SDValue Node = DAG.getTargetExternalSymbol(SymName, PtrVT);
    return DAG.getNode(MidenISD::Wrapper, DL, PtrVT, Node);
  }

  case Intrinsic::miden_shuffle: {
    ...
    return DAG.getNode(MidenISD::SHUFFLE, DL, Op.getValueType(), Ops);
  }
  }
}

SDValue MidenTargetLowering::LowerSIGN_EXTEND_INREG(SDValue Op,
                                                    SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
static SDValue LowerConvertLow(SDValue Op, SelectionDAG &DAG) {
  ...
  auto GetConvertedLane = [](SDValue Op, unsigned &Opcode, SDValue &SrcVec,
    ...
    switch (Op.getOpcode()) {
    case ISD::SINT_TO_FP:
      Opcode = MidenISD::CONVERT_LOW_S;
      break;
    case ISD::UINT_TO_FP:
      Opcode = MidenISD::CONVERT_LOW_U;
      break;
    case ISD::FP_EXTEND:
      Opcode = MidenISD::PROMOTE_LOW;
      break;
    default:
      return false;
    }
  ...
  MVT ExpectedSrcVT;
  switch (LHSOpcode) {
  case MidenISD::CONVERT_LOW_S:
  case MidenISD::CONVERT_LOW_U:
    ExpectedSrcVT = MVT::v4i32;
    break;
  case MidenISD::PROMOTE_LOW:
...
SDValue MidenTargetLowering::LowerBUILD_VECTOR(SDValue Op,
                                               SelectionDAG &DAG) const {
  ...
  if (NumSwizzleLanes >= NumShuffleLanes &&
    ...
    Result =
        DAG.getNode(MidenISD::SWIZZLE, DL, VecT, SwizzleSrc, SwizzleIndices);
    ...
    if ((SplattedLoad = dyn_cast<LoadSDNode>(SplatValue)) &&
      ...
      Result = DAG.getMemIntrinsicNode(
          MidenISD::LOAD_SPLAT, DL, DAG.getVTList(VecT),
...
SDValue MidenTargetLowering::LowerVECTOR_SHUFFLE(SDValue Op,
                                                 SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue MidenTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue MidenTargetLowering::LowerAccessVectorElement(SDValue Op,
                                                      SelectionDAG &DAG) const {
...
SDValue MidenTargetLowering::LowerShift(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  switch (Op.getOpcode()) {
  case ISD::SHL:
    Opcode = MidenISD::VEC_SHL;
    break;
  case ISD::SRA:
    Opcode = MidenISD::VEC_SHR_S;
    break;
  case ISD::SRL:
    Opcode = MidenISD::VEC_SHR_U;
    break;
  default:
...
SDValue MidenTargetLowering::LowerFP_TO_INT_SAT(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
performVectorExtendCombine(SDNode *N, TargetLowering::DAGCombinerInfo &DCI) {
  ...
  unsigned Op =
      IsSext ? (IsLow ? MidenISD::EXTEND_LOW_S : MidenISD::EXTEND_HIGH_S)
             : (IsLow ? MidenISD::EXTEND_LOW_U : MidenISD::EXTEND_HIGH_U);
...
performVectorTruncZeroCombine(SDNode *N, TargetLowering::DAGCombinerInfo &DCI) {
  ...
  auto GetMidenConversionOp = [](unsigned Op) {
    switch (Op) {
    case ISD::FP_TO_SINT_SAT:
      return MidenISD::TRUNC_SAT_ZERO_S;
    case ISD::FP_TO_UINT_SAT:
      return MidenISD::TRUNC_SAT_ZERO_U;
    case ISD::FP_ROUND:
      return MidenISD::DEMOTE_ZERO;
    }
  ...
  if (N->getOpcode() == ISD::CONCAT_VECTORS) {
    ...
    unsigned Op = GetMidenConversionOp(ConversionOp);
    return DAG.getNode(Op, SDLoc(N), ResVT, Source);
  }
...
static SDValue truncateVectorWithNARROW(EVT DstVT, SDValue In, const SDLoc &DL,
  ...
  if (SrcVT.is256BitVector() && DstVT.is128BitVector()) {
    ...
    SDValue Res = DAG.getNode(MidenISD::NARROW_U, DL, OutVT, Lo, Hi);
...
SDValue MidenTargetLowering::PerformDAGCombine(SDNode *N,
