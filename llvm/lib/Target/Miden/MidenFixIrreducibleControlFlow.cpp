//=- MidenFixIrreducibleControlFlow.cpp - Fix irreducible control flow -//
//
...
/// both unnecessary and undesirable for Miden.
///
...
#define DEBUG_TYPE "miden-fix-irreducible-control-flow"
...
class MidenFixIrreducibleControlFlow final : public MachineFunctionPass {
  StringRef getPassName() const override {
    return "Miden Fix Irreducible Control Flow";
  }
...
public:
  ...
  MidenFixIrreducibleControlFlow() : MachineFunctionPass(ID) {}
};

bool MidenFixIrreducibleControlFlow::processRegion(MachineBasicBlock *Entry,
                                                   BlockSet &Blocks,
                                                   MachineFunction &MF) {
  bool Changed = false;
...
// needed.
void MidenFixIrreducibleControlFlow::makeSingleEntryLoop(
  ...
  MachineInstrBuilder MIB =
      BuildMI(Dispatch, DebugLoc(), TII.get(Miden::BR_TABLE_I32));

  // Add the register which will be used to tell the jump table which block to
  // jump to.
  MachineRegisterInfo &MRI = MF.getRegInfo();
  Register Reg = MRI.createVirtualRegister(&Miden::I32RegClass);
  MIB.addReg(Reg);
  ...
  for (auto *Pred : AllPreds) {
    ...
    for (auto *Entry : Pred->successors()) {
      ...
      BuildMI(Routing, DebugLoc(), TII.get(Miden::CONST_I32), Reg)
          .addImm(Indices[Entry]);
      BuildMI(Routing, DebugLoc(), TII.get(Miden::BR)).addMBB(Dispatch);
...
char MidenFixIrreducibleControlFlow::ID = 0;
INITIALIZE_PASS(MidenFixIrreducibleControlFlow, DEBUG_TYPE,
                "Removes irreducible control flow", false, false)

FunctionPass *llvm::createMidenFixIrreducibleControlFlow() {
  return new MidenFixIrreducibleControlFlow();
}

// Test whether the given register has an ARGUMENT def.
static bool hasArgumentDef(unsigned Reg, const MachineRegisterInfo &MRI) {
  for (const auto &Def : MRI.def_instructions(Reg))
    if (Miden::isArgument(Def.getOpcode()))
      return true;
  return false;
}
...
static void addImplicitDefs(MachineFunction &MF) {
  ...
  for (unsigned I = 0, E = MRI.getNumVirtRegs(); I < E; ++I) {
    ...
    BuildMI(Entry, Entry.begin(), DebugLoc(), TII.get(Miden::IMPLICIT_DEF),
            Reg);
  }
...
bool MidenFixIrreducibleControlFlow::runOnMachineFunction(MachineFunction &MF) {
