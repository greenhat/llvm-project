//===-- MidenDebugFixup.cpp - Debug Fixup ------------------===//
//
...
#define DEBUG_TYPE "miden-debug-fixup"

namespace {
class MidenDebugFixup final : public MachineFunctionPass {
  StringRef getPassName() const override { return "Miden Debug Fixup"; }
...
public:
  ...
  MidenDebugFixup() : MachineFunctionPass(ID) {}
};
} // end anonymous namespace

char MidenDebugFixup::ID = 0;
INITIALIZE_PASS(
    MidenDebugFixup, DEBUG_TYPE,
    "Ensures debug_value's that have been stackified become stack relative",
    false, false)

FunctionPass *llvm::createMidenDebugFixup() { return new MidenDebugFixup(); }

bool MidenDebugFixup::runOnMachineFunction(MachineFunction &MF) {
  ...
  for (MachineBasicBlock &MBB : MF) {
        ...
        if (MO.isReg() && MO.getReg().isValid() &&
          ...
          for (auto &Elem : reverse(Stack)) {
            if (MO.getReg() == Elem.Reg) {
              ...
              MO.ChangeToTargetIndex(Miden::TI_OPERAND_STACK, Depth);
        ...
        for (MachineOperand &MO : reverse(MI.explicit_uses())) {
          if (MO.isReg() && MFI.isVRegStackified(MO.getReg())) {
            ...
            assert(Prev.Reg == MO.getReg() &&
                   "MidenDebugFixup: Pop: Register not matched!");
            ...
            if (Prev.DebugValue && !MI.isTerminator()) {
              ...
              BuildMI(*Prev.DebugValue->getParent(), std::next(MII),
                      ...
                      TII->get(Miden::DBG_VALUE), false, Register(),
    ...
    assert(Stack.empty() &&
           "MidenDebugFixup: Stack not empty at end of basic block!");
  }
