...
const SortRegion *SortRegionInfo::getRegionFor(const MachineBasicBlock *MBB) {
  ...
  // MidenException contains BBs in all its subregions (loops or
  ...
  if ((ML && !WE) || (ML && WE && WE->contains(ML->getHeader()))) {
    ...
    ExceptionMap[WE] =
        std::make_unique<ConcreteSortRegion<MidenException>>(WE);
    return ExceptionMap[WE].get();
  }
}
...
MachineBasicBlock *SortRegionInfo::getBottom(const MachineLoop *ML) {
  ...
  for (MachineBasicBlock *MBB : ML->blocks()) {
    ...
    // MidenException contained in this loop, and computes the most bottom
    // BB of them all.
    if (MBB->isEHPad()) {
...
MachineBasicBlock *SortRegionInfo::getBottom(const MidenException *WE) {
